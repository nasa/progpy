...
def run_example():
    ...
    voltage_serial_1 = [
        results_serial_1.outputs[iter]["v"]
        for iter in range(len(results_serial_1.times))
    ]
    voltage_serial_2 = [
        results_serial_2.outputs[iter]["v"]
        for iter in range(len(results_serial_2.times))
    ]
    voltage_serial_3 = [
        results_serial_3.outputs[iter]["v"]
        for iter in range(len(results_serial_3.times))
    ]

    plt.plot(results_orig.times, voltage_orig, "-b", label="Original surrogate")
    plt.plot(
        results_serial_1.times,
        voltage_serial_1,
        "--r",
        label="First JSON serialized surrogate",
    )
    plt.plot(
        results_serial_2.times,
        voltage_serial_2,
        "-.g",
        label="Second JSON serialized surrogate",
    )
    plt.plot(
        results_serial_3.times,
        voltage_serial_3,
        "--y",
        label="Pickled serialized surrogate",
    )
    plt.legend()
    plt.xlabel("Time (sec)")

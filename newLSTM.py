...
# {'window': 28, 'layers': 1, 'units': 35, 'dropout': 0.2922151199006869, 'workers': 3, 'epochs': 12, 'validation_split': 0.23111178028663343}
WINDOW = 28
LAYERS = 1
UNITS = 35
DROPOUT = 0.2922151199006869
WORKERS = 3
EPOCHS = 12
validation_split = 0.23111178028663343
...
def generate_data(m, N):
    ...
    for _ in range(N):
        ...
        results = m.simulate_to_threshold(future_load, dt=('auto', dt), save_freq=SAVE_FREQ, threshold_keys = ['EOD'])
...
for i, z in enumerate(outputs_test):
    ...
    # results = m_lstm.apply_inverse_transformations(m_lstm.u_all, m_lstm.z_all, m_lstm.u_all_shifted, m_lstm.z_all_shifted, 
    #                                                m_lstm.u_all_min_val, 
    #                                                m_lstm.z_all_min_val, 
    #                                                m_lstm.u_all_lambdas, 
    #                                                m_lstm.z_all_lambdas, 
    #                                                m_lstm.u_mean,
    #                                                m_lstm.u_std,
    #                                                m_lstm.z_mean,
    #                                                m_lstm.z_std,
    ...
    #     toe_error = np.inf
    # else:
    toe_error += (times_test[i][-1] - results.times[-1])**2

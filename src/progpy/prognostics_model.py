...
from progpy.utils import ProgressBar, calc_error, input_validation
from progpy.utils.containers import (
    DictLikeMatrixWrapper,
    InputContainer,
    OutputContainer,
)
from progpy.utils.next_state import next_state_functions
...
class PrognosticsModel(ABC):
    ...
    default_parameters = {"process_noise": 0.0, "measurement_noise": 0.0}
    ...
    SimulationResults = namedtuple(
        "SimulationResults", ["times", "inputs", "states", "outputs", "event_states"]
    )
    ...
    def __str__(self) -> str:
        return "{} Prognostics Model (Events: {})".format(
            type(self).__name__, self.events
        )
    ...
    def __setstate__(self, params: dict) -> None:
        ...
        if not isinstance(self.states, abc.Iterable):
            raise TypeError(
                f"model.states must be a list or set, was {type(self.states)}"
            )
        ...
        if not isinstance(self.events, abc.Iterable):
            raise TypeError(
                f"model.events must be a list or set, was {type(self.events)}"
            )
        ...
        if not isinstance(self.outputs, abc.Iterable):
            raise TypeError(
                f"model.outputs must be a list or set, was {type(self.outputs)}"
            )
    ...
    def apply_measurement_noise(self, z):
        ...
        """
        z.matrix += np.random.normal(
            0, self.parameters["measurement_noise"].matrix, size=z.matrix.shape
        )
        return z
    ...
    def apply_process_noise(self, x, dt: float = 1):
        ...
        """
        x.matrix += dt * np.random.normal(
            0, self.parameters["process_noise"].matrix, size=x.matrix.shape
        )
        return x
    ...
    def apply_limits(self, x):
        ...
        for key, limit in self.state_limits.items():
            if np.any(np.array(x[key]) < limit[0]):
                warn(
                    "State {} limited to {} (was {})".format(key, limit[0], x[key]),
                    ProgModelStateLimitWarning,
                )
                x[key] = np.maximum(x[key], limit[0])
            if np.any(np.array(x[key]) > limit[1]):
                warn(
                    "State {} limited to {} (was {})".format(key, limit[1], x[key]),
                    ProgModelStateLimitWarning,
                )
    ...
    def output(self, x):
        ...
        if self.is_direct:
            warn_once(
                "This Direct Model does not support output estimation. Did you mean to call time_of_event?"
            )
        else:
    ...
    def event_state(self, x) -> dict:
        ...
        return {
            key: 1.0 - float(t_met) for (key, t_met) in self.threshold_met(x).items()
        }
    ...
    def threshold_met(self, x) -> dict:
        ...
        return {
            key: event_state <= 0 for (key, event_state) in self.event_state(x).items()
        }
    ...
    def is_state_transition_model(self) -> bool:
        ...
        has_integrator_next_state = (
            type(self).next_state in next_state_functions.values()
        )
        has_overridden_transition = (
            not (has_default_next_state or has_integrator_next_state)
            or type(self).dx != PrognosticsModel.dx
        )
    ...
    def time_of_event(
        self, x, future_loading_eqn=lambda t, x=None: {}, **kwargs
    ) -> dict:
        """
        .. versionadded:: 1.5.0
    ...
    def simulate_to(
        self,
        time: float,
        future_loading_eqn: abc.Callable = lambda t, x=None: {},
        first_output=None,
        **kwargs,
    ) -> namedtuple:
        """
        ...
        if not isinstance(time, Number) or time < 0:
            raise ValueError(
                "'time' must be positive, was {} (type: {})".format(time, type(time))
            )
    ...
    def simulate_to_threshold(
        self,
        future_loading_eqn: abc.Callable = None,
        first_output=None,
        events: list = None,
        **kwargs,
    ) -> namedtuple:
        ...
        if first_output and not all(key in first_output for key in self.outputs):
            raise ValueError(
                "Missing key in 'first_output', must have every key in model.outputs"
            )
        ...
        if "threshold_keys" in kwargs:
            ...
            if events is None:
                ...
                warn(
                    "Both `events` and `threshold_keys` were set. `events` will be used."
                )
        ...
        if not isinstance(config["dt"], (Number, tuple, str)) and not callable(
            config["dt"]
        ):
            raise TypeError(
                "'dt' must be a number or function, was a {}".format(type(config["dt"]))
            )
        if isinstance(config["dt"], Number) and config["dt"] < 0:
            raise ValueError("'dt' must be positive, was {}".format(config["dt"]))
        if not isinstance(config["save_freq"], Number) and not isinstance(
            config["save_freq"], tuple
        ):
            raise TypeError(
                "'save_freq' must be a number, was a {}".format(
                    type(config["save_freq"])
                )
            )
        if (isinstance(config["save_freq"], Number) and config["save_freq"] < 0) or (
            ...
            raise ValueError(
                "'save_freq' must be positive, was {}".format(config["save_freq"])
            )
        if not isinstance(config["save_pts"], abc.Iterable):
            raise TypeError(
                "'save_pts' must be list or array, was a {}".format(
                    type(config["save_pts"])
                )
            )
        if not isinstance(config["horizon"], Number):
            raise TypeError(
                "'horizon' must be a number, was a {}".format(type(config["horizon"]))
            )
        if config["horizon"] < 0:
            raise ValueError(
                "'horizon' must be positive, was {}".format(config["horizon"])
            )
        if config["x"] is not None and not all(
            [state in config["x"] for state in self.states]
        ):
            raise ValueError("'x' must contain every state in model.states")
        if "thresholds_met_eqn" in config and not callable(
            config["thresholds_met_eqn"]
        ):
            raise TypeError(
                "'thresholds_met_eqn' must be callable (e.g., function or lambda)"
            )
        if (
            "thresholds_met_eqn" in config
            and config["thresholds_met_eqn"].__code__.co_argcount != 1
        ):
            raise ValueError(
                "'thresholds_met_eqn' must accept one argument (thresholds)-> bool"
            )
        if (
            "event_strategy" in config
            and callable(config["event_strategy"])
            and config["event_strategy"].__code__.co_argcount != 1
        ):
            raise ValueError(
                "'event_strategy' callable must accept one argument (thresholds)-> bool"
            )
        if not isinstance(config["print"], bool):
            raise TypeError(
                "'print' must be a bool, was a {}".format(type(config["print"]))
            )
        ...
        if config["event_strategy"] in ("first", "any"):
            ...
            raise ValueError(
                f"Invalid value for `event_strategy`: {config['event_strategy']}. Should 'all', 'first', or a callable (e.g., function or lambda)"
            )

        if "thresholds_met_eqn" in config:
            warn(
                "thresholds_met_eqn will be removed after version 1.7 of ProgPy. The thresholds_met_eqn argument can now be passed into event_strategy.",
                DeprecationWarning,
                stacklevel=2,
            )
        ...
        if (
            len(events) == 0
            and config.get("thresholds_met_eqn", None) is None
            and "horizon" not in kwargs
        ):
            raise ValueError(
        ...
        if config["print"]:
            ...
            def update_all():
                ...
                saved_states.append(
                    deepcopy(x)
                )  # Avoid optimization where x is not copied
        ...
        if dt_mode == "constant":
            ...
            def next_time(t, x=None):
                next_save_pt = (
                    save_pts[save_pt_index]
                    if save_pt_index < len(save_pts)
                    else float("inf")
                )
                next_eval_pt = (
                    eval_pts[eval_pt_index]
                    if eval_pt_index < len(eval_pts)
                    else float("inf")
                )
                opts = (
                    item
                    for item in (dt, next_save - t, next_save_pt - t, next_eval_pt - t)
                    if item > 0
                )
                return min(*opts)
        ...
        while t < horizon:
            ...
            t_load = (
                t + dt_i / 2
            )  # Saving as separate variable reduces likelihood of floating point error
        ...
        if not saved_outputs:
            ...
            saved_event_states = LazySimResult(
                self.event_state, saved_times, saved_states
            )
        else:
    ...
    def calc_error(
        ...
        **kwargs,
        ...
        if not all(
            isinstance(obj, tuple(acceptable_types)) for obj in [times, inputs, outputs]
        ):
            type_error = (
                f"Types passed in must be from the following: Sequence, np.ndarray, SimResult, or LazySimResult. Current types"
        ...
        if len(times) != len(inputs) or len(inputs) != len(outputs):
            len_error = (
                f"Times, inputs, and outputs must all be the same length. Current lengths"
                f"{(' at data location (' + str(_loc) + ')' if _loc is not None else '')}"
        ...
        if len(times) < 2:
            less_2_error = (
                f"Must provide at least 2 data points for times, inputs, and outputs"
                f"{(' at data location (' + str(_loc) + ')' if _loc is not None else '')}."
            )
            raise ValueError(less_2_error)

        # Determines if all values of arguments are iterables
        input_validation.all_none_iterable(
            times, "times", loc=_loc
        ) if _loc is not None else input_validation.all_none_iterable(times, "times")
        input_validation.all_none_iterable(
            inputs, "inputs", loc=_loc
        ) if _loc is not None else input_validation.all_none_iterable(inputs, "inputs")
        input_validation.all_none_iterable(
            outputs, "outputs", loc=_loc
        ) if _loc is not None else input_validation.all_none_iterable(
            outputs, "outputs"
        )
        ...
        if kwargs["stability_tol"] > 1 or kwargs["stability_tol"] <= 0:
            raise ValueError(
                f"Configurable cutoff must be some float value in the domain (0, 1]. Received {kwargs['stability_tol']}."
            )
        ...
        if dt <= 0:
            raise ValueError(
                f"Keyword argument 'dt' must a initialized to a value greater than 0. Currently passed in {dt}."
            )

        if "x0" in kwargs.keys() and not isinstance(
            kwargs["x0"], (self.StateContainer, dict)
        ):
            raise TypeError(
                f"Keyword argument 'x0' must be initialized to a Dict or StateContainer, not a {type(kwargs['x0']).__name__}."
            )
    ...
    def estimate_params(
        ...
        inputs: List[InputContainer] = None,
        outputs: List[OutputContainer] = None,
        ...
        **kwargs,
        ...
        if keys is None:
            ...
            keys = [
                ...
                if isinstance(self.parameters[key], Number)
            ]

        if isinstance(keys, set):
            raise ValueError(
                "Can not pass in keys as a Set. Sets are unordered by construction, so bounds may be out of order."
            )
            ...
            if isinstance(key, (tuple, list)):
                ...
                for key_element in key:
                    if not isinstance(tmp, Mapping) or key_element not in tmp:
                        raise ValueError(
                            f"Key '{keys_so_far}[{key_element}]' not in model parameters"
                        )
        ...
        if (
            isinstance(times, set)
            or isinstance(inputs, set)
            or isinstance(outputs, set)
        ):
            raise TypeError(
                "Times, inputs, and outputs cannot be a set. Sets are unordered by definition, so passing in arguments as sets may result in incorrect behavior."
            )
        ...
        if runs is None:
            ...
            if len(times) != len(inputs) or len(inputs) != len(outputs):
                raise ValueError(
                    f"Times, inputs, and outputs must be same length. Length of times: {len(times)}, Length of inputs: {len(inputs)}, Length of outputs: {len(outputs)}"
                )
            if len(times) == 0:
        ...
        if isinstance(config["bounds"], dict):
            ...
            for key in config["bounds"].keys():
                if key not in self.parameters:
                    warn(
                        f"{key} is not a valid parameter (i.e., it is not a parameter present in this model) and should not be passed in to the bounds"
                    )
            config["bounds"] = [
                config["bounds"].get(key, (-np.inf, np.inf)) for key in keys
            ]
        else:
            if not isinstance(config["bounds"], abc.Iterable):
                raise ValueError(
                    "Bounds must be a tuple of tuples or a dict, was {}".format(
                        type(config["bounds"])
                    )
                )
            if len(config["bounds"]) != len(keys):
                raise ValueError(
                    f"Bounds must be same length as keys. There were {len(config['bounds'])} Bounds given whereas there are {len(keys)} Keys. To define partial bounds, use a dict (e.g., {{'param1': {(0, 5)}, 'param3': {(-5.5, 10)}}})"
                )
        for bound in config["bounds"]:
            if isinstance(bound, set):
                raise TypeError(
                    f"The Bound {bound} cannot be a Set. Sets are unordered by construction, so bounds may be out of order."
                )
            if (not isinstance(bound, abc.Iterable)) or (len(bound) != 2):
                raise ValueError(
                    "Each bound must be a tuple of format (lower, upper), was {}".format(
                        type(config["bounds"])
                    )
                )
        ...
        # Set noise to 0
        m_noise, self.parameters["measurement_noise"] = (
        ...
        for i, (times, inputs, outputs) in enumerate(runs):
            ...
            if len(times) != len(inputs) or len(inputs) != len(outputs):
                raise ValueError(
                    f"Times, inputs, and outputs must be same length for the run at index {i}. Length of times: {len(times)}, Length of inputs: {len(inputs)}, Length of outputs: {len(outputs)}"
                )
            if len(times) == 0:
                raise ValueError(
                    f"Times, inputs, and outputs for Run {i} must have at least one element"
                )
        ...
        def optimization_fcn(params):
            ...
            for run in runs:
                try:
                    err += self.calc_error(
                        run[0], run[1], run[2], method=config["error_method"], **kwargs
                    )
                except Exception:
                    return 1e99
    ...
    def generate_surrogate(
        self, load_functions: List[abc.Callable], method: str = "dmd", **kwargs
    ):
        """
        ...
        """
        from progpy.data_models import SURROGATE_METHOD_LOOKUP

        if method not in SURROGATE_METHOD_LOOKUP.keys():
            raise ValueError(
                "Method {} not supported. Supported methods: {}".format(
                    method, SURROGATE_METHOD_LOOKUP.keys()
                )
            )

    ...
    def data(self):
        if self.__data == None:
            ...
            for i in range(len(self.times)):
                ...
                # Flatten
                sigma_pt_tranformed = array(
                    [array(list(sigma_pt.values())) for sigma_pt in sigma_pt_tranformed]
                )  # map -> array

                # Apply Unscented Transform to form output distribution
                mean, cov = self.__ut_fcn(
                    sigma_pt_tranformed, self.__sigma_fcn.Wm, self.__sigma_fcn.Wc
                )
...
class UnscentedTransformPredictor(Predictor):
    ...
    def __init__(self, model, **kwargs):
        ...
        self.sigma_points = kalman.MerweScaledSigmaPoints(
            num_states,
    ...
    def predict(
        self, state, future_loading_eqn: Callable = None, events=None, **kwargs
    ) -> PredictionResults:
        """
        ...
        if (
            ...
            or isinstance(state, self.model.StateContainer)
            or isinstance(state, ScalarData)
        ):
            raise TypeError(
                "state must be a distribution (e.g., MultivariateNormalDist, UnweightedSamples), not scalar"
            )
        ...
        if params["event_strategy"] != "all":
            raise ValueError(
                f"`event_strategy` {params['event_strategy']} not supported. Currently, only 'all' event strategy is supported"
            )
        ...
        # Update State
        self.__state_keys = state_keys = (
            state.mean.keys()
        )  # Used to maintain ordering as we strip keys and return
        ...
        save_pt_index = 0
        ToE = {
            key: [float("nan") for i in range(n_points)] for key in events
        }  # Keep track of final ToE values
        last_state = {
            key: [None for i in range(n_points)] for key in events
        }  # Keep track of final state values
        ...
        def update_all():
            times.append(t)
            inputs.append(
                deepcopy(self.__input)
            )  # Avoid optimization where u is not copied
            x_dict = MultivariateNormalDist(
                self.__state_keys, filt.x, filt.P, _type=self.model.StateContainer
            )
        ...
        while t < params["horizon"]:
            ...
            t += dt
            mean_state = StateContainer(
                {key: x for (key, x) in zip(state_keys, filt.x)}
            )
            ...
            for i, point in zip(range(n_points), points):
                ...
                for key in events:
                    if t_met[key]:
                        ...
                        all_failed = (
                            False  # This event for this sigma point hasn't been met yet
                        )
            if all_failed:
        ...
        for event_key in last_state.keys():
            if any([last_state_i is None for last_state_i in last_state[event_key]]):
                ...
                continue
            last_state_pts = array(
                [
                    [last_state_i[state_key] for state_key in state_keys]
                    for last_state_i in last_state[event_key]
                ]
            )
            # last_state_pts = transpose(last_state_pts)
            last_state_mean, last_state_cov = kalman.unscented_transform(
                last_state_pts, sigma_points.Wm, sigma_points.Wc
            )
            final_state[event_key] = MultivariateNormalDist(
                state_keys,
                last_state_mean,
                last_state_cov,
        ...
        output_prediction = LazyUTPrediction(
            state_prediction, sigma_points, kalman.unscented_transform, model.output
        )
        event_state_prediction = LazyUTPrediction(
            state_prediction,
            sigma_points,

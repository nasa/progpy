...
class ToEPredictionProfile(UserDict):
    ...
    def prognostic_horizon(
        self, criteria_eqn, ground_truth, **kwargs
    ) -> Dict[str, float]:
        """
    ...
    def plot(
        self, ground_truth: dict = None, alpha: float = None, show: bool = True
    ) -> dict:  # use ground truth, alpha if given,
        ...
        for t, v in self.items():
            ...
            for key in v.keys():
                ...
                result_figs[key].get_axes()[0].scatter(
                    [t] * len(samples), samples, color="red"
                )  # Adding single distribution of estimates
        ...
        if ground_truth:  # If ground_truth is specified, add ground_truth to each event plot (green line)
            for key, val in ground_truth.items():
                ...
                if alpha:  # if ground_truth and alpha are specified, add alpha bounds (faded green highlight)
                    result_figs[key].get_axes()[0].fill_between(
                        gt_x,
                        np.array(gt_y) * (1 - alpha),
                        np.array(gt_y) * (1 + alpha),
                        color="green",
                        alpha=0.2,
                    )

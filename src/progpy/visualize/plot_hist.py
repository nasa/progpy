...
def plot_hist(
    samples: array, fig: Figure = None, keys: List[str] = None, **kwargs
) -> Figure:
    """Create a histogram
    ...
    if keys is not None:
        ...
        for key in keys:
            if key not in samples[0].keys():
                raise TypeError(
                    "Key {} was not present in samples (keys: {})".format(
                        key, list(samples[0].keys())
                    )
                )
    else:
        keys = samples[0].keys()
    ...
    for key in keys:
        ax.hist(
            [sample[key] for sample in samples if sample[key] is not None],
            label=key,
            **kwargs,
        )

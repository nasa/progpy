...
def lqr_calc_k(A, B, Q, R):
    ...
    K = np.real(
        K
    )  # some spurious imaginary parts (1e-13) sometimes remain in the matrix. we manually remove them
...
class LQR:
    ...
    def __init__(self, x_ref, vehicle, **kwargs):
        ...
        if not isinstance(x_ref, dict):
            raise TypeError(
                "Reference trajectory must be a dictionary of numpy arrays for each state throughout time."
            )
        ...
        self.states = (
            vehicle.states
        )  # state variables of the system to be controlled (x, y, z, phi, theta, psi)
        self.n_states = (
            len(self.states) - 2
        )  # number of states (minus two to remove time and mission_complete)
        self.inputs = vehicle.inputs  # input variables of the system to be controlled
        self.n_inputs = (
            len(self.inputs) - 1
        )  # number of inputs (minus one to remove mission_complete)
        ...
        self.parameters = dict(
            ...
            ),  # state error penalty matrix: how 'bad' is an error in the state vector w.r.t. the reference state vector
            R=np.diag(
                [500, 4000, 4000, 4000]
            ),  # input penalty matrix: how 'hard' it is to produce the desired input (thrust and three moments along three axes)
            scheduled_var="psi",  # variable used to create the scheduled controller gains (only psi allowed for now)
            index_scheduled_var=5,
        )  # index corresponding to the scheduled_var (psi) in the state vector x; i.e., x[5] = psi
        self.parameters.update(kwargs)  # update control parameters according to user

        # Get scheduled variable index
        self.parameters["index_scheduled_var"] = self.states.index(
            self.parameters["scheduled_var"]
        )
    ...
    def __call__(self, t, x=None):
        ...
        time_ind = np.argmin(
            np.abs(t_k - self.ref_traj["t"].tolist())
        )  # get index of time value in ref_traj closest to t_k
        x_ref_k = []
        ...
        scheduled_var = x_k[
            self.parameters["index_scheduled_var"]
        ]  # get psi from current state vector (self.parameters = 'psi')
        k_idx = np.argmin(
            np.abs(
                self.scheduled_states[self.parameters["index_scheduled_var"], :]
                - scheduled_var
            )
        )  # find the psi value stored in the controller closest to the current psi --> extract index
        K = self.control_gains[
            :, :, k_idx
        ]  # extract gain corresponding to the current psi value
        u = self.compute_input(
            K, error
        )  # compute input u given the gain matrix K and the error between current and reference state
        u[0] += self.ss_input
    ...
    def build_scheduled_control(
        self,
        system_linear_model_fun,
        input_vector,
        state_vector_vals=None,
        index_scheduled_var=None,
    ):
        if state_vector_vals is None:
            ...
            state_vector_vals[index_scheduled_var, :] = np.linspace(
                -2.0 * np.pi, 2.0 * np.pi, n_schedule_grid
            )

        n, m = state_vector_vals.shape
        assert n == self.n_states, (
            "number of states set at initialization and size of state_vector_vals mismatch."
        )
        self.control_gains = np.zeros(
            (self.control_gains.shape[0], self.control_gains.shape[1], m)
        )
...
class LQR_I(LQR):
    ...
    def __init__(self, x_ref, vehicle, **kwargs):
        ...
        self.outputs = vehicle.outputs[
            :3
        ]  # output variables of the system to be controlled (x, y, z only)
        ...
        self.Ai = np.zeros(
            (self.n_states + self.n_outputs, self.n_states + self.n_outputs)
        )
        ...
        self.parameters["Qi"] = np.diag(
            np.concatenate(
                (np.diag(self.parameters["Q"]), self.parameters["qi"]), axis=0
            )
        )
        ...
        self.control_gains = np.zeros(
            (self.n_inputs, self.n_states + self.n_outputs, 1)
        )
    ...
    def compute_gain(self, A, B):
        ...
        self.K, self.E = lqr_calc_k(
            self.Ai, self.Bi, self.parameters["Qi"], self.parameters["Ri"]
        )
        return self.K, self.E
    ...
    def compute_input(self, gain, error):
        ...
        err_integral = (
            np.sum(
                err_hist[
                    :, max([0, len(self.err_hist) - self.parameters["int_lag"]]) :
                ],
                axis=1,
            )
            * self.dt
        )
        return -np.dot(gain[:, : self.n_states], error) - np.dot(
            gain[:, self.n_states :], err_integral
        )

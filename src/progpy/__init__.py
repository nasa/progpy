...
def run_prog_playback(obs, pred, future_loading, output_measurements, **kwargs):
    warnings.warn(
        "Depreciated in 1.2.0, will be removed in a future release.", DeprecationWarning
    )
    config = {  # Defaults
    ...
    for t, measurement in output_measurements:
        ...
        if t >= next_predict:
            pred_results = pred.predict(
                obs.x.sample(config["num_samples"]),
                future_loading,
    ...
    return predictors.PredictionResults(
        times, inputs, states, outputs, event_states, toes
    )

...
def all_none_iterable(iter, name, loc=None):
    ...
    """
    count = sum(
        isinstance(element, abc.Sequence) for element in iter
    )  # Number of iterables in each iterable
    if 0 < count < len(iter):
        if loc:
            raise ValueError(
                f"Some, but not all elements, are iterables for argument {name} at data location {loc}."
            )
        else:
            raise ValueError(
                f"Some, but not all elements, are iterables for argument {name}."
            )

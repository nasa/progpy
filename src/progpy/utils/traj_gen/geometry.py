...
def greatcircle_distance(lat1, lat2, lon1, lon2, R=6371e3):
    ...
    a = np.sin(dphi / 2.0) * np.sin(dphi / 2.0) + np.cos(phi1) * np.cos(phi2) * np.sin(
        dlambda / 2.0
    ) * np.sin(dlambda / 2.0)
...
def vincenty_distance(p1, p2, tol=1e-12, max_iter=200):
    ...
    b = (
        (1.0 - f) * a
    )  # semi-minor axis of the ellipsoid (radius at the poles), meters, according to WGS84 = 6356752.314245

    # Define coordinate-dependent values
    U1 = np.arctan(
        (1.0 - f) * np.tan(lat1)
    )  # reduced latitude (latitude on auxiliary sphere, lat1)
    U2 = np.arctan(
        (1.0 - f) * np.tan(lat2)
    )  # reduced latitude (latitude on auxiliary sphere, lat2)
    ...
    while iter < max_iter:
        ...
        sin_sigma = np.sqrt(
            (cos_U2 * sin_lam) ** 2.0
            + (cos_U1 * sin_U2 - sin_U1 * cos_U2 * cos_lam) ** 2.0
        )
        if sin_sigma == 0.0:
...
def geodetic_distance(lats, lons, alts, method="greatcircle", return_surf_vert=False):
    ...
    if len(lats) != 2 or len(lons) != 2 or len(alts) != 2:
        raise ValueError(
            "Latitudes, longitudes, and altitude values must be 2-element lists or arrays."
        )

    if (
        type(alts[0]) == np.ndarray
    ):  # if altitudes are vectors, compute point-wise difference (must be the same length)
...
def heading_compute_geodetic(lat, lon):
    ...
    for jj in range(1, n):
        ...
        Y = np.cos(lat[jj - 1]) * np.sin(lat[jj]) - np.sin(lat[jj - 1]) * np.cos(
            lat[jj]
        ) * np.cos(dlon_)  # compute cartesian coordinate
...
class Coord:
    ...
    def __init__(self, lat0, lon0, alt0):
        ...
        self.alt0 = alt0
        self.N0 = self.a / np.sqrt(
            1 - self.e**2.0 * np.sin(self.lat0) ** 2.0
        )  # [m], Radius of curvature on the Earth
    ...
    def ecef2enu(self, xecef, yecef, zecef):
        ...
        y = (
            -np.cos(self.lon0) * np.sin(self.lat0) * xd
            - np.sin(self.lat0) * np.sin(self.lon0) * yd
            + np.cos(self.lat0) * zd
        )
        z = (
            np.cos(self.lat0) * np.cos(self.lon0) * xd
            + np.cos(self.lat0) * np.sin(self.lon0) * yd
            + np.sin(self.lat0) * zd
        )
    ...
    def enu2ecef(self, xenu, yenu, zenu):
        ...
        xd = (
            -np.sin(self.lon0) * xenu
            - np.cos(self.lon0) * np.sin(self.lat0) * yenu
            + np.cos(self.lat0) * np.cos(self.lon0) * zenu
        )
        yd = (
            np.cos(self.lon0) * xenu
            - np.sin(self.lat0) * np.sin(self.lon0) * yenu
            + np.cos(self.lat0) * np.sin(self.lon0) * zenu
        )
    ...
    def ecef2geodetic(self, X, Y, Z):
        ...
        lat = np.arctan2(
            (Z + epsilon * b * np.sin(q) ** 3.0),
            (p - self.e**2.0 * self.a * np.cos(q) ** 3.0),
        )  # [rad], latitude
        lon = np.arctan2(Y, X)  # [rad], longitude
        N = self.a / np.sqrt(
            1.0 - self.e**2.0 * np.sin(lat) ** 2.0
        )  # [m], Radius of curvature on the Earth
        alt = (p / np.cos(lat)) - N
        return lat, lon, alt
    ...
    def geodetic2ecef(self, lat, lon, alt):
        ...
        N = self.a / np.sqrt(
            1.0 - self.e**2.0 * np.sin(lat) ** 2.0
        )  # [m], Radius of curvature on the Earth, phi = latitude

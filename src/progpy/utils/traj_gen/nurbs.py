...
class NURBS:
    def __init__(self, points: dict, weights, times, yaw=None, **kwargs) -> None:
        self.points = points
    ...
    def generate(self, timestep_size):
        ...
        points_new, self.times_new, self.yaw_new, self.weight_vector_new = (
            self.generate_intermediate_points()
        )
        self.points_new = {
            dim: points_new[:, i] for i, dim in enumerate(self.dims)
        }  # Save way-points with intermediate values:
        ...
        for ii, dim_i in enumerate(self.dims):
            pos_interp, timevec = self.interpolate(
                pos=points_new[:, ii],
        ...
        # Interpolate yaw
        # -----------------
        yaw_interp, timevec = self.interpolate(
            self.yaw_new,  # anchor points: yaw points
            self.times_new,  # time instants of the anchor points
            np.ones_like(
                self.yaw_new
            ),  # weight vector, can be all the same for the yaw points
            ...
            interp_order="zero",
        )  # order of the yaw curves: the yaw should be kept constant in-between waypoints (no smooth variations, except for the proximity of turns), keep order 0 so is step-wise constant
    ...
    def gen_knot_vector(self, n):
        ...
        msk1 = (idx_vec >= k) * (
            idx_vec <= n
        )  # create a mask for values between k and n
        t[msk1] = (idx_vec[msk1] - 1) - k + 1  # assign values to the masked elements
        t[idx_vec > n] = (
            n - k + 2
        )  # assign fixed value n-k+2 for everything above index n
    ...
    def evaluate(self, pointvec, weightvec, order=None, basis_length=None):
        ...
        U = np.linspace(
            self.knotv[order - 1],
            self.knotv[-1],
            int(round(abs(self.knotv[-1] - self.knotv[order - 1]) * basis_length)),
        )
        q = len(U)
        ...
        N[0, 0] = (
            1.0  # Numerical issue: the first point of the first basis is not computed but remains 0 when it should be 1. This fixes it.
        )

        S = np.dot(N, weightvec)  # Calculate denominator of rational basis function
        R = N * np.repeat(
            weightvec.reshape((1, -1)), repeats=N.shape[0], axis=0
    ...
    def interpolate(
        ...
        pos_vs_time = self.evaluate(
            p_vector, weightvector, order=order, basis_length=basis_length
        )
    ...
    def generate_intermediate_points(
        ...
        intermediate_points = np.zeros(
            (points.shape[0] - 1, 3)
        )  # initialize array of fictitious waypoints
        for ii in range(1, points.shape[0]):
            intermediate_points[ii - 1, :] = (
                points[ii, :] - points[ii - 1, :]
            ) / 2.0 + points[
                ii - 1, :
            ]  # generate a fictitious waypoint as average of true waypoints

        # Generate the new waypoint matrix
        points_new = np.zeros(
            (points.shape[0] + intermediate_points.shape[0], points.shape[1])
        )
        ...
        for jj in range(len(weight_vector_new)):
            if jj % 2 == 0:
                weight_vector_new[jj] = self.weights[
                    counter_1
                ]  # Assign pre-defined weight to the waypoint
                counter_1 += 1
            else:
                weight_vector_new[jj] = (
                    1  # Assign standard weight of 1 to the fictitious waypoint
                )

        # -------- Generate new ETA vector -------------
        intermediate_times = np.zeros(
            (len(self.weights) - 1,)
        )  # initialize fictitious ETA vector
        for ii in range(1, len(intermediate_times) + 1):
            intermediate_times[ii - 1] = (times[ii] - times[ii - 1]) / 2.0 + times[
                ii - 1
            ]  # generate fictitious ETA as average of real ETAs

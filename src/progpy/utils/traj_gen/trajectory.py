...
def compute_derivatives(position_profile, timevec):
    ...
    for key in dim_keys:
        vel_interp[key], acc_interp[key], _ = derivate_position(
            position_profile[key], timevec[1] - timevec[0]
        )
...
class Trajectory:
    ...
    def __init__(self, lat, lon, alt, takeoff_time=None, etas=None, **kwargs):
        # Check waypoint types:
        # ---------------------
        if (
            not isinstance(lat, np.ndarray)
            or not isinstance(lon, np.ndarray)
            or not isinstance(alt, np.ndarray)
        ):
            raise TypeError(
                "Latitudes, longitudes, and altitudes must be provided as n x 1 arrays."
            )
        if lat.shape != lon.shape or lon.shape != alt.shape:
            raise ValueError(
                "Provided latitude, longitude, and altitude arrays must be the same length."
            )
        if lat.shape[0] <= 1 or lon.shape[0] <= 1 or alt.shape[0] <= 1:
            raise ValueError(
                "Latitudes, longitudes, and altitudes must be provided as n x 1 arrays, with n > 1."
            )
        if etas is not None:
            ...
            if len(etas) != 1 and len(etas) != lat.shape[0]:
                raise ValueError(
                    "ETA must be either a take off time (one value), or a vector array with same length as lat, lon and alt."
                )
            for eta in etas:
        ...
        self.waypoints["heading"] = geom.gen_heading_angle(
            self.waypoints["lat"], self.waypoints["lon"], self.waypoints["alt"]
        )

        # Set up coordinate system conversion between Geodetic,
        # Earth-Centric Earth-Fixed (ECF), and Cartesian (East-North-Up, ENU)
        self.coordinate_system = geom.Coord(lat[0], lon[0], alt[0])

        # Define speed parameters - only necessary if ETAs are not defined
        if etas is not None and (
            "cruise_speed" in kwargs
            or "ascent_speed" in kwargs
            or "descent_speed" in kwargs
            or "landing_speed" in kwargs
        ):
            warn(
                "Speed values are ignored since ETAs were specified. To define speeds (cruise, ascent, descent, landing) instead, do not specify ETAs."
            )
        if etas is None and (
            "cruise_speed" not in kwargs
            or "ascent_speed" not in kwargs
            or "descent_speed" not in kwargs
            or "landing_speed" not in kwargs
        ):
        ...
        self.waypoints["x"], self.waypoints["y"], self.waypoints["z"] = (
            self.coordinate_system.geodetic2enu(
                self.waypoints["lat"], self.waypoints["lon"], self.waypoints["alt"]
            )
        )
        ...
        if self.parameters["weight_vector"] is None:
            self.parameters["weight_vector"] = np.asarray(
                [
                    self.parameters["waypoint_weight"],
                ]
                * len(self.waypoints["x"])
            )
    ...
    def compute_trajectory_nurbs(self, dt) -> None:
        ...
        # Store as attribute
        self.trajectory = {
            **{"position": np.vstack(list(pos_interp.values())).T},
            **{"velocity": np.vstack(list(linear_profiles["velocity"].values())).T},
            **{
                "acceleration": np.vstack(
                    list(linear_profiles["acceleration"].values())
                ).T
            },
            **angular_profiles,
            **{"time": time_interp},
    ...
    def generate(self, dt, **kwargs):
        ...
        assert len(self.parameters["weight_vector"]) == len(self.waypoints["x"]), (
            "Length of waypoint weight vector and number of waypoints must coincide."
        )
        ...
        (
            ...
            self.trajectory["geodetic_pos"][:, 2],
        ) = self.coordinate_system.enu2geodetic(
    ...
    def __adjust_eta_given_max_acceleration(self, dt) -> None:
        ...
        while keep_adjusting:
            ...
            for i in range(n - 1):
                dist1 = geom.euclidean_distance_point_vector(
                    position_values[i, :], self.trajectory["position"]
                )
                dist2 = geom.euclidean_distance_point_vector(
                    position_values[i + 1, :], self.trajectory["position"]
                )

                delta_etas = etas_rel[i + 1] - etas_rel[i]
                dtime1 = (
                    100.0 * np.abs(etas_rel[i] - self.trajectory["time"]) ** 3.0
                )  # time is fundamental and more important than distance, so I'm using a higher power to take that into account
                dtime2 = (
                    100.0 * np.abs(etas_rel[i + 1] - self.trajectory["time"]) ** 3.0
                )

                accelerations_abs = np.abs(
                    self.trajectory["acceleration"][
                        max(np.argmin(dist1 * dtime1), 0) : min(
                            np.argmin(dist2 * dtime2), m
                        ),
                        :,
                    ]
                )
            ...
            if counter == maxiter:
                ...
                warn(
                    "max number of iterations reached, the trajectory still contains accelerations beyond the limit."
                )
                break

    def set_landing_waypoints(
        self,
    ):
        """
        ...
        idx_land = np.asarray(
            self.waypoints["alt"] < self.speed_parameters["landing_altitude"]
        )
        ...
        if idx_land_pos.size != 0:
            ...
            for item in idx_land_pos:
                if item == 0:  # if first element is below, just add a landing waypoint
                    self.waypoints["lat"] = np.insert(
                        self.waypoints["lat"], item + 1, self.waypoints["lat"][item]
                    )
                    self.waypoints["lon"] = np.insert(
                        self.waypoints["lon"], item + 1, self.waypoints["lon"][item]
                    )
                    if n_times > 1:
                        t_land = linearinterp_t(
                            self.waypoints["eta"][item],
                            self.waypoints["alt"][item],
                            self.waypoints["eta"][item + 1],
                            self.waypoints["alt"][
                                item + 1
                            ],  # just added the landing altitude, so I need to take the next point
                            self.speed_parameters["landing_altitude"],
                        )
                        self.waypoints["eta"] = np.insert(
                            self.waypoints["eta"], item + 1, t_land
                        )
                    self.waypoints["alt"] = np.insert(
                        self.waypoints["alt"],
                        item + 1,
                        self.speed_parameters["landing_altitude"] * 1.0,
                    )
                    counter += 1

                elif (
                    item == n_waypoints - 1
                ):  # if one before the last element, add landing waypoints right before landing.
                    if (
                        ...
                        self.waypoints["lat"] = np.insert(
                            self.waypoints["lat"],
                            -1,
                            self.waypoints["lat"][item + counter - 1],
                        )
                        self.waypoints["lon"] = np.insert(
                            self.waypoints["lon"],
                            -1,
                            self.waypoints["lon"][item + counter - 1],
                        )
                        if n_times > 1:
                            t_land = linearinterp_t(
                    ...
                    self.waypoints["lat"] = np.insert(
                        self.waypoints["lat"],
                        item + counter + idx_delta,
                        self.waypoints["lat"][item + counter],
                    )
                    self.waypoints["lon"] = np.insert(
                        self.waypoints["lon"],
                        item + counter + idx_delta,
                        self.waypoints["lon"][item + counter],
                    )
                    if n_times > 1:
                        t_land = linearinterp_t(
                            self.waypoints["eta"][item + counter + idx_delta - 1],
                        ...
                        self.waypoints["eta"] = np.insert(
                            self.waypoints["eta"], item + counter + idx_delta, t_land
                        )
                    self.waypoints["alt"] = np.insert(
                        self.waypoints["alt"],
                        item + counter + idx_delta,
                    ...
                    if idx_delta == 0:
                        ...
                        if (
                            ...
                            if self.waypoints["eta"] is not None:
                                t_land = linearinterp_t(
                                    self.waypoints["eta"][item + counter],
                                    self.waypoints["alt"][item + counter],
                                    self.waypoints["eta"][item + counter + 1],
                                    self.waypoints["alt"][item + counter + 1],
                                    self.speed_parameters["landing_altitude"],
                                ...
                                self.waypoints["eta"] = np.insert(
                                    self.waypoints["eta"], item + counter + 1, t_land
                                )
    ...
    def set_eta(self, idx_land_pos, hovering=0) -> None:
        ...
        self.speed_parameters["cruise_speed"] = reshape_route_attribute(
            self.speed_parameters["cruise_speed"], dim=n - 1, msk=idx_land_pos
        )
        self.speed_parameters["ascent_speed"] = reshape_route_attribute(
            self.speed_parameters["ascent_speed"], dim=n - 1, msk=idx_land_pos
        )
        self.speed_parameters["descent_speed"] = reshape_route_attribute(
            self.speed_parameters["descent_speed"], dim=n - 1, msk=idx_land_pos
        )
        self.speed_parameters["landing_speed"] = reshape_route_attribute(
            self.speed_parameters["landing_speed"], dim=n - 1, msk=idx_land_pos
        )
        ...
        if len(self.waypoints["alt"]) <= 2:
            raise ValueError(
                "At least 3 waypoints are required to compute ETAS from speed. Only {} were given.".format(
                    len(self.lat)
                )
            )
        ...
        for point in range(
            n
        ):  # this for loop could be improved by calculating distances all at once.
            dh, dv = geom.geodetic_distance(
                [self.waypoints["lat"][point], self.waypoints["lat"][point + 1]],
                [self.waypoints["lon"][point], self.waypoints["lon"][point + 1]],
                [self.waypoints["alt"][point], self.waypoints["alt"][point + 1]],
                method="greatcircle",
                return_surf_vert=True,
            )
            dv = dv[0]
            ...
            if (
                dv > 0
                and alt_for_land[point] > self.speed_parameters["landing_altitude"]
            ):
                ...
                vert_speed = 0.0  # not moving vertically.

            if etas is None:
                # Define the correct speed:
                if np.isclose(dh + dv, 0.0):
                    d_eta[point] = (
                        2.0  # if there's no vertical / horizontal speed (waypoints are identical) add a default hovering value of 2 s to avoid extreme accelerations.
                    )
                else:
                    if np.isclose(dh, 0.0):
                        speed_sq = vert_speed * vert_speed
                    elif np.isclose(dv, 0.0):
                        speed_sq = self.speed_parameters["cruise_speed"][point] ** 2.0
                    else:
                        speed_sq = (
                            self.speed_parameters["cruise_speed"][point] ** 2.0
                            + vert_speed * vert_speed
                        )
                    d_eta[point] = (
                        np.sqrt((dh**2.0 + dv**2.0) / speed_sq) * 1.3
                    )  # adding some %

                    # If speed is larger than desired (possible when both dh, dv>0), increment d_eta to reduce until desired (consider margin)
                    while dh / d_eta[point] > (
                        self.speed_parameters["cruise_speed"][point]
                        * (1.0 + cruise_speed_margin)
                    ) or dv / d_eta[point] > (vert_speed * (1.0 + vert_speed_margin)):
                        d_eta[point] += 2.0
            else:
        ...
        self.waypoints["eta"] = [
            eta + self.waypoints["takeoff_time"] for eta in eta_array
        ]

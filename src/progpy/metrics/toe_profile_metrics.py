...
def alpha_lambda(
    toe_profile: ToEPredictionProfile,
    ground_truth: dict,
    lambda_value: float,
    alpha: float,
    beta: float,
        ...
        if t_prediction >= lambda_value:
            ...
            if params["print"]:
                for key in keys:
                    ...
                    print(
                        "\tBounds: [{} - {}]({}%)".format(
                            bounds[key][0], bounds[key][1], pib[key]
                        )
                    )
            return result
...
def prognostic_horizon(
    ...
    **kwargs,
    ...
    ph_result = {
        k: None for k in ground_truth.keys()
    }  # False means not yet met; will be either a numerical value or None if met
    for t_prediction, toe in toe_profile.items():
        ...
        criteria_eqn_dict = criteria_eqn(
            tte, ground_truth_tte
        )  # -> dict[event_names as str, bool]
        for k, v in criteria_eqn_dict.items():
            if v and (ph_result[k] == None):
                ph_calc = ground_truth[k] - t_prediction
                if ph_calc > 0:
                    ph_result[k] = (
                        ph_calc  # PH = EOL - ti # ground truth is a dictionary {'EOD': 3005.2} should be ph_result[k] = g_truth[key] - t_prediction
                    )
...
def cumulative_relative_accuracy(
    toe_profile: ToEPredictionProfile, ground_truth: dict, **kwargs
) -> Dict[str, float]:
    """

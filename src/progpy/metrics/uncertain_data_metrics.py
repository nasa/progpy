...
def calc_metrics(
    data: UncertainData, ground_truth: Union[float, dict] = None, **kwargs
) -> dict:
    ...
    if isinstance(data, UncertainData):
        ...
        result = {
            key: calc_metrics(
                ...
                ground_truth
                if not ground_truth
                else ground_truth[key],  # If ground_truth is a dict, use key
                **kwargs,
            )
            for key in keys
        }
        ...
        if isscalar(data[0]) or data[0] is None:
            ...
            raise TypeError(
                "Data must be type Uncertain Data or array of dicts, was {}".format(
                    type(data)
                )
            )
    else:
    ...
    if len(data_abridged) < len(data):
        warn(
            "Some samples were None, resulting metrics only consider non-None samples. Note: in some cases, this will bias the metrics."
        )
    data_abridged.sort()
    ...
    metrics = {
        "min": data_abridged[0],
        "percentiles": {
            "0.01": data_abridged[int(len(data_abridged) / 10000)]
            if len(data_abridged) >= 10000
            else None,
            "0.1": data_abridged[int(len(data_abridged) / 1000)]
            if len(data_abridged) >= 1000
            else None,
            "1": data_abridged[int(len(data_abridged) / 100)]
            if len(data_abridged) >= 100
            else None,
            "10": data_abridged[int(len(data_abridged) / 10)]
            if len(data_abridged) >= 10
            else None,
            "25": data_abridged[int(len(data_abridged) / 4)]
            if len(data_abridged) >= 4
            else None,
            "50": median,
            "75": data_abridged[int(3 * len(data_abridged) / 4)]
            if len(data_abridged) >= 4
            else None,
        },
        "median": median,
        ...
        "max": data_abridged[-1],
        "median absolute deviation": sum([abs(x - median) for x in data_abridged])
        / len(data_abridged),
        "mean absolute deviation": sum([abs(x - m) for x in data_abridged])
        / len(data_abridged),
    ...
    if ground_truth is not None:
        ...
        metrics["mean absolute error"] = sum(
            [abs(x - ground_truth) for x in data_abridged]
        ) / len(data_abridged)
        metrics["mean absolute percentage error"] = (
            metrics["mean absolute error"] / ground_truth
        )
        metrics["relative accuracy"] = (
            1 - abs(ground_truth - metrics["mean"]) / ground_truth
        )
        metrics["ground truth percentile"] = stats.percentileofscore(
            data_abridged, ground_truth
        )

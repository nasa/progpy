...
from progpy import LinearModel, PrognosticsModel
...
class DMDModel(LinearModel, DataModel):
    ...
    A subclass of :py:class:`progpy.LinearModel` and :py:class:`progpy.data_models.DataModel` that uses Dynamic Mode Decomposition to simulate a system throughout time.
    ...
    def __init__(self, dmd_matrix, *_, **kwargs):
        ...
        self.states = (
            params["state_keys"] + params["output_keys"] + params["event_keys"]
        )
        ...
        self.parameters["dmd_matrix"] = (
            dmd_matrix  # This simplifies pickling (all data in parameters)
        )

        if "x0" in self.parameters and not isinstance(
            self.parameters["x0"], self.StateContainer
        ):
            self.parameters["x0"] = self.StateContainer(params["x0"])

    @classmethod
    def from_data(
        cls, times, inputs, outputs, states=None, event_states=None, **kwargs
    ):
        """
        ...
        Keyword Args:
            trim_data_to (float, optional):
                Fraction (0-1) of data resulting from :py:func:`progpy.PrognosticsModel.simulate_to_threshold` used to train DMD surrogate model
        ...
        Additionally, other keyword arguments from :py:func:`progpy.PrognosticsModel.simulate_to_threshold`
        ...
        # Input validation
        if (
            not isinstance(config["trim_data_to"], Number)
            or config["trim_data_to"] > 1
            or config["trim_data_to"] <= 0
        ):
            raise ValueError(
                "Invalid 'trim_data_to' input value, must be between 0 and 1."
            )
        if (
            not isinstance(config["stability_tol"], Number)
            or config["stability_tol"] < 0
        ):
            raise ValueError(
                f"Invalid 'stability_tol' input value {config['stability_tol']}, must be a positive number."
            )
        if (
            not isinstance(config["training_noise"], Number)
            or config["training_noise"] < 0
        ):
            raise ValueError(
                f"Invalid 'training_noise' input value {config['training_noise']}, must be a positive number."
            )
        ...
        if config["event_keys"] is None:
            if event_states is None:
                ...
                if config["event_keys"] is None:
                    # Wasn't able to fill it in
                    config["event_keys"] = [
                        f"es{i}" for i in range(len(event_states[0][0]))
                    ]

        for state_key in config["state_keys"]:
            if (
                state_key in config["input_keys"]
                or state_key in config["output_keys"]
                or state_key in config["event_keys"]
            ):
                config["state_keys"].remove(state_key)
                warn(
                    f"State value '{state_key}' is duplicated in inputs, outputs, or events; duplicate has been removed."
                )
        ...
        for run in range(n_runs):
            ...
            if len(u) != len(z):
                raise ValueError(
                    f"Must have same number of steps for inputs and outputs in a single run. Not true for run {run}"
                )
            if len(u) == 0:
                raise ValueError(
                    f"Each run must have at least one timestep, not true for Run {run}"
                )
            ...
            if states != None:
                ...
                if len(x) != len(u):
                    raise ValueError(
                        f"Must have same number of steps for inputs, states, and outputs in a single run. Not true for states in run {run}"
                    )
                if isinstance(x, SimResult):
            ...
            if event_states is not None:
                ...
                if len(es) != len(u):
                    raise ValueError(
                        f"Must have same number of steps for inputs, event_states, and outputs in a single run. Not true for event_states in run {run}"
                    )
                if isinstance(es, SimResult):
        ...
        if "x0" not in config:
            config["x0"] = np.mean(
                np.array(
                    [x[: -n_inputs if n_inputs != 0 else None, 0] for x in x_list]
                ),
                axis=0,
            )[np.newaxis].T
        ...
        eig_val, _ = np.linalg.eig(
            dmd_matrix[:, 0 : -n_inputs if n_inputs > 0 else None]
        )
        ...
        if sum(eig_val > 1) != 0:
            for eig_val_i in eig_val:
                if eig_val_i > 1 and (eig_val_i - 1) > config["stability_tol"]:
                    warn(
                        "The DMD matrix is unstable, may result in poor approximation."
                    )

        del config["dt"]

        # Build Model
        return cls(dmd_matrix, dt=config["save_freq"], **config)
    ...
    def from_model(cls, m, load_functions, **kwargs):
        ...
        config = {
            ...
            "measurement_noise_dist": m.parameters.get(
                "measurement_noise_dist", "normal"
            ),
    ...
    def simulate_to_threshold(
        ...
        results = super().simulate_to_threshold(
            future_loading_eqn, first_output, events, **kwargs_sim
        )
        ...
        # Default parameters
        config = {"dt": None, "save_freq": None, "save_pts": []}
        config.update(kwargs)

        if (
            config["save_freq"] == self.dt
            or (
                isinstance(config["save_freq"], tuple)
                and config["save_freq"][0] % self.dt < 1e-9
                and config["save_freq"][1] == self.dt
            )
        ) and config["save_pts"] == []:
        ...
        if config["save_freq"] is not None:
            if isinstance(config["save_freq"], tuple):
                ...
                t_start = config["save_freq"][0]
                start = max(
                    t_start, results.times[0] - (results.times[0] - t_start) % t_step
                )
                time_array = np.arange(start + t_step, results.times[-1], t_step)
            else:
                time_array = np.arange(
                    results.times[0] + config["save_freq"],
                    results.times[-1],
                    config["save_freq"],
                )
        ...
        for states_name in self.states:
            states_list_temp = [
                results.states[iter1a][states_name]
                for iter1a in range(len(results.states))
            ]
            states_dict_temp[states_name] = interp1d(results.times, states_list_temp)(
                time_interp
            )
        states_interp = [
            self.StateContainer(
                {key: state[i] for key, state in states_dict_temp.items()}
            )
        ...
        for inputs_name in self.inputs:
            inputs_list_temp = [
                results.inputs[iter1a][inputs_name]
                for iter1a in range(len(results.inputs))
            ]
            inputs_dict_temp[inputs_name] = interp1d(results.times, inputs_list_temp)(
                time_interp
            )
        inputs_interp = [
            self.InputContainer(
                {key: input[i] for key, input in inputs_dict_temp.items()}
            )
            for i in range(len(time_interp))
        ]
        ...
        return self.SimulationResults(
            time_interp, inputs, states, outputs, event_states
        )

...
class LSTMStateTransitionModel(DataModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        input_shape = (
            output_model.input.shape if state_model is None else state_model.input.shape
        )
        ...
        if event_state_model is not None:
            ...
            if t_met_model is not None and t_met_model.output.shape[-1] / 2 != n_events:
                raise ValueError(
                    "t_met output length must be twice that of event state"
                )
        ...
        self.events = kwargs.get(
            "event_keys",
            getattr(self, "events", [f"event{i}" for i in range(n_events)]),
    ...
    def __getstate__(self):
        warn_once(
            "LSTMStateTransitionModel uses a Keras model, which does not always support pickling. We recommend that you use the keras save and load model functions instead with m.model",
            RuntimeWarning,
        )
    ...
    def next_state(self, x, u, _):
        ...
        states = x.matrix[
            len(input_data) : -self.parameters["state_model"].output_shape[1]
        ]
        ...
        if states[0, 0] is None:
            return self.StateContainer(
                np.vstack(
                    (
                        states,
                        x.matrix[-self.parameters["state_model"].output_shape[1] :],
                    )
                )
            )
        else:
    ...
    def output(self, x):
        if x.matrix[0, 0] is None:
            warn(
                f"Output estimation is not available until at least {1 + self.parameters['window']} timesteps have passed."
            )
        ...
        if self.parameters["state_model"] is None:
            ...
            internal_states = x.matrix[
                -self.parameters["state_model"].output_shape[1] :
            ].T
    ...
    def event_state(self, x):
        ...
        if x.matrix[0, 0] is None:
            warn(
                f"Event state estimation is not available until at least {1 + self.parameters['window']} timesteps have passed."
            )
            return {key: None for key in self.events}
    ...
    def threshold_met(self, x):
        ...
        if x.matrix[0, 0] is None:
            warn(
                f"Threshold met estimation is not available until at least {1 + self.parameters['window']} timesteps have passed."
            )
            return {key: None for key in self.events}
        ...
        m_t_met = [
            np.argmax(m_t_met[0][i * 2 : (i + 1) * 2]) == 0
    ...
    def summary(self, file=sys.stdout, expand_nested=False, show_trainable=False):
        ...
        if self.parameters["state_model"] is not None:
            ...
            self.parameters["state_model"].summary(
                print_fn=file.write,
        ...
        self.parameters["output_model"].summary(
            print_fn=file.write,
            expand_nested=expand_nested,
            show_trainable=show_trainable,
        )
        ...
        if self.parameters["event_state_model"] is not None:
            ...
            self.parameters["event_state_model"].summary(
                print_fn=file.write,
                expand_nested=expand_nested,
                show_trainable=show_trainable,
            )
    ...
    def pre_process_data(
        inputs, outputs, event_states=None, t_met=None, window=10, **kwargs
    ):
        """
        ...
        for i in range(len(inputs)):
            ...
            if len(u) > 0 and len(u) <= window:
                raise TypeError(
                    f"Not enough data for window size {window}. Only {len(u)} elements present."
                )
            ...
            if isinstance(u, (list, np.ndarray)):
                if len(u) == 0:
                    ...
                    u_i = [
                        [[u[i + j]] for j in range(1, window + 1)]
                    ...
                    u_i = [
                        [
                            [u[i + j][k] for k in range(n_inputs)]
                    ...
                    raise TypeError(
                        f"Unsupported input type: {type(u)} for internal element (data[0][i]"
                    )
            else:
                raise TypeError(
                    f"Unsupported data type: {type(u)}. input u must be in format List[Tuple[np.array, np.array]] or List[Tuple[SimResult, SimResult]]"
                )
            ...
            if isinstance(z, (list, np.ndarray)):
                if len(z) != len(u) and len(u) != 0 and len(z) != 0:
                    ...
                    raise IndexError(
                        f"Number of outputs ({len(z)}) does not match number of inputs ({len(u)})"
                    )
                ...
                if len(z) == 0:
                    ...
                    n_outputs = len(z[0])
                    z_i = [
                        [z[i][k] for k in range(n_outputs)]
                        for i in range(window, len(z))
                    ]
                else:
                    raise TypeError(
                        f"Unsupported input type: {type(z)} for internal element (output[i])"
                    )
                ...
                raise TypeError(
                    f"Unsupported data type: {type(z)}. output z must be in format List[Tuple[np.array, np.array]] or List[Tuple[SimResult, SimResult]]"
                )
            ...
            if event_states is not None:
                ...
                if isinstance(es, SimResult):
                    if len(es[0].keys()) == 0:
                        ...
                        es = np.array(
                            [[es_i[key] for key in es_i.keys()] for es_i in es]
                        )
                ...
                if isinstance(es, (list, np.ndarray)):
                    if len(es) != len(u) and len(u) != 0 and len(es) != 0:
                        ...
                        # triggering this check
                        raise IndexError(
                            f"Number of event_states ({len(es)}) does not match number of inputs ({len(u)})"
                        )
                    ...
                    if len(es) == 0:
                        ...
                        n_events = len(es[0])
                        es_i = [
                            [es[i][k] for k in range(n_events)]
                            for i in range(window, len(es))
                        ]
                    else:
                        raise TypeError(
                            f"Unsupported input type: {type(es)} for internal element (es[i])"
                        )

                else:
                    raise TypeError(
                        f"Unsupported data type: {type(es)}. event state must be in format List[Tuple[np.array, np.array]] or List[Tuple[SimResult, SimResult]]"
                    )
            else:
                es_i = []
            ...
            if t_met is not None:
                ...
                if isinstance(t, (list, np.ndarray)):
                    if len(t) != len(u) and len(u) != 0 and len(t) != 0:
                        ...
                        # triggering this check
                        raise IndexError(
                            f"Number of t_met ({len(t)}) does not match number of inputs ({len(u)})"
                        )
                    ...
                    if len(t) == 0:
                        ...
                        # Output is 1-d array (i.e., 1 output)
                        t_i = [
                            [1, 0] if t[i] else [0, 1] for i in range(window, len(t))
                        ]
                        ...
                        raise TypeError(
                            f"Unsupported input type: {type(t[0])} for internal element (t[i])"
                        )

                else:
                    raise TypeError(
                        f"Unsupported data type: {type(t)}. t_met must be in format List[Tuple[np.array, np.array]] or List[Tuple[SimResult, SimResult]]"
                    )
            else:
                t_i = []
    ...
    def from_data(cls, inputs, outputs, event_states=None, t_met=None, **kwargs):
        ...
        if not np.isscalar(params["window"]):
            raise TypeError(
                f"window must be an integer greater than 0, not {type(params['window'])}"
            )
        if params["window"] <= 0:
            raise ValueError(f"window must be greater than 0, got {params['window']}")
        if not np.isscalar(params["layers"]):
            raise TypeError(
                f"layers must be an integer greater than 0, not {type(params['layers'])}"
            )
        if params["layers"] <= 0:
        ...
        if not isinstance(params["units"], (abc.Sequence, np.ndarray)):
            raise TypeError(
                f"units must be a Sequence (e.g., list or tuple) of integers, not {type(params['units'])}"
            )
        if len(params["units"]) != params["layers"]:
            raise ValueError(
                f"units must be a list of integers of length {params['layers']}, got {params['units']}"
            )
        for i in range(params["layers"]):
            if params["units"][i] <= 0:
                raise ValueError(
                    f"units[{i}] must be greater than 0, got {params['units'][i]}"
                )
        if not isinstance(params["dropout"], Number):
            raise TypeError(
                f"dropout must be an float greater than or equal to 0, not {type(params['dropout'])}"
            )
        if params["dropout"] < 0:
            raise ValueError(
                f"dropout must be greater than or equal to 0, got {params['dropout']}"
            )
        if not isinstance(params["activation"], (list, tuple, np.ndarray)):
            params["activation"] = [
                params["activation"] for _ in range(params["layers"])
            ]
        if not np.isscalar(params["validation_split"]):
            raise TypeError(
                f"validation_split must be an float between 0 and 1, not {type(params['validation_split'])}"
            )
        if params["validation_split"] < 0 or params["validation_split"] >= 1:
            raise ValueError(
                f"validation_split must be between 0 and 1, got {params['validation_split']}"
            )
        if not np.isscalar(params["epochs"]):
            raise TypeError(
                f"epochs must be an integer greater than 0, not {type(params['epochs'])}"
            )
        if params["epochs"] < 1:
        ...
        if not isinstance(params["normalize"], bool):
            raise TypeError(
                f"normalize must be a boolean, not {type(params['normalize'])}"
            )

        # Prepare datasets
        (u_all, z_all, es_all, t_all) = LSTMStateTransitionModel.pre_process_data(
            inputs, outputs, event_states=event_states, t_met=t_met, **params
        )
        ...
        for i in range(params["layers"]):
            if i == params["layers"] - 1:
                # Last layer
                x = keras.layers.LSTM(
                    params["units"][i], activation=params["activation"][i]
                )(x)
            else:
                # Intermediate layer
                x = keras.layers.LSTM(
                    params["units"][i],
                    activation=params["activation"][i],
        ...
        outputs = [
            keras.layers.Dense(z_all.shape[1] if z_all.ndim == 2 else 1, name="output")(
                x
            )
        ]
        output_data = [z_all]
        ...
        if event_states is not None:
            outputs.append(
                keras.layers.Dense(
                    es_all.shape[1] if es_all.ndim == 2 else 1, name="event_state"
                )(x)
            )
        ...
        if t_met is not None and t_all.shape[1] > 0:
            ...
            t_met_layers = [
                keras.layers.Dense(2, activation="softmax") for _ in range(n_events)
            ]
            ...
            if len(t_met_layers) == 1:
                ...
                # Concatenate layers
                outputs.append(
                    keras.layers.Concatenate(name="t_met")(t_met_layers_output)
                )
        ...
        n_state_layers = (
            params["layers"] + 1 + (params["dropout"] > 0) + (params["normalize"])
        )
        output_layer_input = keras.layers.Input(
            model.layers[n_state_layers - 1].output.shape[1:]
        )
    ...
    def simulate_to_threshold(
        ...
        while x.matrix[0, 0] is None:
            if "horizon" in kwargs and t > kwargs["horizon"]:
                raise Exception(
                    "Not enough timesteps to reach minimum number of steps for model simulation"
                )
            dt = next_time(t, x)
            t = t + dt / 2
        ...
        if "horizon" in kwargs:
            if kwargs["horizon"] < t:
                raise ValueError(
                    "Prediction horizon does not allow enough steps to fully initialize model"
                )
            kwargs["horizon"] = kwargs["horizon"] - t
        return super().simulate_to_threshold(

...
class CentrifugalPumpBase(PrognosticsModel):
    ...
    default_parameters = {  # Set to defaults
        ...
        "HOil3": 1.5,
        # Parameter limits
        "lim": {"A": 9.5, "Tt": 370, "Tr": 370, "To": 350},
        # Wear Rates
        "wA": 0,
        "wRadial": 0,
        "wThrust": 0,
    ...
    def initialize(self, u, z=None):
        ...
        x0["QLeak"] = (
            self.parameters["cLeak"]
            * self.parameters["ALeak"]
            * np.sqrt(abs(u["psuc"] - u["pdisch"]))
            * np.sign(u["psuc"] - u["pdisch"])
        )
    ...
    def next_state(self, x, u, dt: float):
        ...
        Todot = (
            ...
            / params["mcOil"]
            * (
                params["HOil1"] * (x["Tt"] - x["To"])
                + params["HOil2"] * (x["Tr"] - x["To"])
        ...
        Trdot = (
            ...
            * (
                x["rRadial"] * x["w"] * x["w"]
                - params["HRadial1"] * (x["Tr"] - u["Tamb"])
                - params["HRadial2"] * (x["Tr"] - x["To"])
            )
        )
        ...
        Te = (
            ...
            * params["p"]
            * params["R2"]
            / (slip * (u["wsync"] + 0.00001))
            * u["V"] ** 2
            / (
                (params["R1"] + params["R2"] / slip) ** 2
                + (u["wsync"] * params["L1"]) ** 2
            )
        )
        backTorque = (
    ...
    def output(self, x):
        ...
        return self.OutputContainer(
            {"w": x["w"], "Qout": Qout, "Tt": x["Tt"], "Tr": x["Tr"], "To": x["To"]}
        )

    def event_state(self, x) -> dict:
        return {
            "ImpellerWearFailure": (x["A"] - self.parameters["lim"]["A"])
            / (self.parameters["x0"]["A"] - self.parameters["lim"]["A"]),
            "ThrustBearingOverheat": (self.parameters["lim"]["Tt"] - x["Tt"])
            / (self.parameters["lim"]["Tt"] - self.parameters["x0"]["Tt"]),
            "RadialBearingOverheat": (self.parameters["lim"]["Tr"] - x["Tr"])
            / (self.parameters["lim"]["Tr"] - self.parameters["x0"]["Tr"]),
            "PumpOilOverheat": (self.parameters["lim"]["To"] - x["To"])
            / (self.parameters["lim"]["To"] - self.parameters["x0"]["To"]),
        }
...
def OverwrittenWarning(params):
    ...
    """
    warnings.warn(
        "wA, wRadial, and wThrust will be overwritten within the model, since the wear rates are part of the state. Use CentrifugalPumpBase to remove this behavior."
    )
    return {}
...
class CentrifugalPumpWithWear(CentrifugalPumpBase):
    ...
    default_parameters["x0"].update({"wA": 0.0, "wThrust": 0, "wRadial": 0})

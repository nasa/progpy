...
def update_B(params):
    ...
    return {"B": 0.0051 * params["Km"] ** 1.9485}


def update_J(params):
    # J is the sum of Jl (load) and Js (shaft)
    return {"J": params["Jl"] + params["Js"]}
...
class DCMotorSP(PrognosticsModel):
    ...
    * t       time, :math:`s`
    ...
    def dx(self, x, u):
        ...
        dvrotdt = (
            1.0 / inertia * (el_torque - load_torque - friction_coeff * x["v_rot"])
        )

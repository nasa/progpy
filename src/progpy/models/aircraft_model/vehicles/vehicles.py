...
from progpy.models.aircraft_model.vehicles.control import allocation_functions as caf
...
def TAROT18(payload=0.0, gravity=9.81):
    ...
    """
    mass = dict(
        body_empty=3.2,  # kg, total empty mass
        max_payload=5.0,  # kg, admissible payload
        max_tow=11.2,  # kg, maximum takeoff weight
        arm=0.23
        + 0.11
        + 0.0373
        + 0.03,  # kg, arm (comprehensive of motors). 230g motor, 110g ESC, 37.3g propeller.
        ...
        Izz=None,
    )  # kg*m^2, inertia moments along local axis

    geom = dict(
        num_rotors=8,  # , number of rotors
        body_type="thickdisk",  # -, should be either sphere, flatdisk, thickdisk.
        body_radius=((0.301 + 0.209) / 2.0)
        / 2.0,  # m, body radius. Internal plate is kind of oval, took average of main axes
        body_height=0.025 * 2.0,  # m, height of the body
        arm_length=1.270 / 2.0 - 0.1275,
    )  # m, arm length

    dynamics = dict(
        num_states=12,  # -, number of states in state vector: [x, y, z, u, v, w, phi, theta, psi, p, q, r]
        ...
        thrust2weight=(45.0 / gravity * geom["num_rotors"])
        / (
            mass["body_empty"] + mass["arm"] * geom["num_rotors"]
        ),  # -, thrust over weight ratio. Tarot 18: 50 N per motor divided by weight
        ...
        aero=dict(
            cd=1.0,  # drag coefficient of airframe (including rotors), not reliable
...
def DJIS1000(payload=0.0, gravity=9.81):
    ...
    mass = dict(
        body_empty=1.8,  # kg, total empty mass
...
def rotorcraft_inertia(m, g):
    ...
    if g["body_type"].lower() == "sphere":
        ...
        raise Exception(
            "Body geometry not implemented. Please choose among: sphere, flatdisk, thickdisk."
        )

    m["Ixx"] = Ix0 + 2.0 * sum(
        m["arm"] * motor_distance_from_xaxis**2.0
    )  # [kg m^2], inertia along x
    m["Iyy"] = m["Ixx"]  # [kg m^2], inertia along y (symmetric uav)
    m["Izz"] = Iz0 + g["num_rotors"] * (
        g["arm_length"] ** 2.0 * m["arm"]
    )  # [kg m^2], inertia along z
    return m, g
...
def rotorcraft_masses(mass_dict, geom_dict):
    ...
    """
    mass_dict["body"] = (
        mass_dict["body_empty"] + geom_dict["num_rotors"] * mass_dict["arm"]
    )

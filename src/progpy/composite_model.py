...
class CompositeModel(PrognosticsModel):
        ...
        for m in models:
            ...
            if isinstance(m, Iterable):  # Already a tuple
                ...
                if not isinstance(m[0], str):
                    raise ValueError(
                        "The first element of each model tuple must be a string"
                    )
            ...
            if isinstance(m[1], PrognosticsModel):
                ...
                raise ValueError(
                    "The second element of each model tuple must be a PrognosticsModel"
                )
    ...
    def __setstate__(self, params: dict) -> None:
        ...
        for name, m in params["models"]:
            ...
            self.performance_metric_keys |= set(
                [name + DIVIDER + p for p in m.performance_metric_keys]
            )

        for name, fcn in params["functions"]:
            self.inputs |= set(
                [name + DIVIDER + u for u in signature(fcn).parameters.keys()]
            )
        ...
        # Handle Connections
        self.__to_input_connections = {m_name: [] for m_name in params["model_names"]}
        self.__to_state_connections = {m_name: [] for m_name in params["model_names"]}
        ...
        for connection in params["connections"]:
            ...
            if in_model not in params["model_names"]:
                raise ValueError(
                    "The input model must be one of the models in the composite model"
                )
            if out_model not in params["model_names"]:
                raise ValueError(
                    "The output model must be one of the models in the composite model"
                )
            ...
            if in_key in self.states:
                ...
                # Add to state (to preserve last between runs)
                self.__to_state_from_pm_connections[out_model].append(
                    (in_key_part, in_key)
                )
                self.states.add(in_key)
            else:
                raise ValueError(f"The input key {in_key} must be an output or state")
        ...
        class PassthroughParams:
            ...
            def __call__(self, params: dict) -> dict:
                params["models"][self.model_index][1].parameters[self.key] = params[
                    self.combined_key
                ]
                return {}
        ...
        for name, m in params["models"]:
            ...
            for key in m.parameters.keys():
                ...
                if combined_key in self.param_callbacks:
                    self.param_callbacks[combined_key].append(
                        PassthroughParams(params["models"], name, key)
                    )
                else:
                    self.param_callbacks[combined_key] = [
                        PassthroughParams(params["models"], name, key)
                    ]

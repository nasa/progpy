...
def load_data(batt_id: str) -> tuple:
    ...
    desc = {
        ...
        "runs": [
            {"type": str(run_type[0]), "desc": str(desc[0]), "date": str(date[0])}
            for (run_type, desc, date) in zip(
                result["step"][0, 0]["type"][0],
                result["step"][0, 0]["comment"][0],
                result["step"][0, 0]["date"][0],
            )
        ],
    }
    ...
    result = [
        pd.DataFrame(
            np.array(
                [
                    result[key][0, i][0]
                    for key in ("relativeTime", "current", "voltage", "temperature")
                ],
                np.float64,
            ).T,

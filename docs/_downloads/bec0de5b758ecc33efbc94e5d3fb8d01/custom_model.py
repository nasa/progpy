...
def run_example():
    ...
    future_loading_eqns = [
        lambda t, x=None: batt.InputContainer({"i": 1 + 1.4 * load})
        for load in range(6)
    ]
    ...
    (u_all, z_all, _, _) = LSTMStateTransitionModel.pre_process_data(
        input_data, output_data, window=12
    )
    ...
    history = model.fit(
        u_all, z_all, epochs=30, callbacks=callbacks, validation_split=0.1
    )
    ...
    m_custom = LSTMStateTransitionModel(
        ...
        input_keys=["i", "dt"],
        output_keys=["t", "v"],
        history=history,  # Provide history so plot_history will work
    )

...
def run_example():
    ...
    batt.parameters["process_noise"] = {
        key: PROCESS_NOISE * value for key, value in x0.items()
    }
    z0 = batt.output(x0)
    ...
    x0 = MultivariateNormalDist(
        x0.keys(),
        list(x0.values()),
        np.diag([max(1e-9, X0_COV * abs(x)) for x in x0.values()]),
    )
        ...
        for row in reader:
            ...
            if step % PREDICTION_UPDATE_FREQ == 0:
                mc_results = mc.predict(
                    filt.x, future_loading, t0=t, n_samples=NUM_SAMPLES, dt=TIME_STEP
                )
        ...
        def criteria_eqn(tte: UncertainData, ground_truth_tte: dict) -> dict:
            ...
            for key, value in ground_truth_tte.items():
                ...
                bounds[key] = [
                    value - alpha_calc,
                    value + alpha_calc,
                ]  # Construct bounds for all events
            ...
            return {
                key: percentage_in_bounds[key] > BETA
                for key in percentage_in_bounds.keys()
            }

...
class TestDiscreteStates(unittest.TestCase):
    ...
    def test_tank(self):
        ...
        def stupid_load(t, x=None):
            ...
            if x is None:
                return m.InputContainer(
                    {"q_in": 0.1, "valve_command": ValveState.closed}
                )
            return m.InputContainer({"q_in": 0.1, "valve_command": x["valve"]})

        result = m.simulate_to_threshold(
            stupid_load, events="full", save_freq=1, horizon=25
        )
        ...
        result = m.simulate_to_threshold(
            smart_load, events="full", save_freq=1, horizon=25
        )
...
def main():
    ...
    result = runner.run(
        load_test.loadTestsFromTestCase(TestDiscreteStates)
    ).wasSuccessful()

...
from progpy.models.test_models.linear_models import (
    OneInputNoOutputNoEventLM,
    OneInputOneOutputNoEventLM,
    OneInputTwoStatesNoOutputNoEventLM,
...
class MockModel:
    ...
    inputs = ["i1", "i2"]
    outputs = ["o1"]
    default_parameters = {"p1": 1.2, "x0": {"a": 1, "b": 5, "c": -3.2, "t": 0}}
...
class MockProgModel(MockModel, PrognosticsModel):
    ...
    def event_state(self, x):
        t = x["t"]
        return {"e1": max(1 - t / 5.0, 0), "e2": max(1 - t / 15.0, 0)}
...
class TestModels(unittest.TestCase):
    ...
    def test_non_container(self):
        ...
        m = MockProgModelStateDict(
            process_noise_dist="none", measurement_noise_dist="none"
        )
        ...
        (times, inputs, states, outputs, event_states) = m.simulate_to_threshold(
            load, {"o1": 0.8}, dt=0.5, save_freq=1.0
        )
    ...
    def test_integration_type(self):
        ...
        m_rk4_integration = LinearThrownObject(
            integration_method="rk4", process_noise=0, measurement_noise=0
        )
    ...
    def test_parameters_statelikematrixwrapper(self):
        ...
        m1 = OneInputTwoStatesNoOutputNoEventLM(
            process_noise=m0.parameters["process_noise"]
        )
    ...
    def test_integration_type_scipy(self):
        ...
        m = LinearThrownObject(
            integration_method=RK45, process_noise=0, measurement_noise=0
        )
        x = m.initialize()
        u = m.InputContainer({})
        ...
        # RK23
        m = LinearThrownObject(
            integration_method=RK23, process_noise=0, measurement_noise=0
        )
        x = m.initialize()
        u = m.InputContainer({})
        x = m.next_state(x, u, 0.1)
        ...
        # DOP853
        m = LinearThrownObject(
            integration_method=DOP853, process_noise=0, measurement_noise=0
        )
        x = m.initialize()
        u = m.InputContainer({})
        x = m.next_state(x, u, 0.1)
        ...
        # Radau
        m = LinearThrownObject(
            integration_method=Radau, process_noise=0, measurement_noise=0
        )
        x = m.initialize()
        u = m.InputContainer({})
        x = m.next_state(x, u, 0.1)
        ...
        # BDF
        m = LinearThrownObject(
            integration_method=BDF, process_noise=0, measurement_noise=0
        )
        x = m.initialize()
        u = m.InputContainer({})
        x = m.next_state(x, u, 0.1)
        ...
        # LSODA
        m = LinearThrownObject(
            integration_method=LSODA, process_noise=0, measurement_noise=0
        )
        x = m.initialize()
        u = m.InputContainer({})
        x = m.next_state(x, u, 0.1)
    ...
    def test_size(self):
        ...
        # The difference is overhead for the dict
        diff = size4 - size3
        sum_of_parts = sys.getsizeof("test_list") + sys.getsizeof(
            [79534, 84392, 93333, -243934, 23233]
        )
        self.assertLess(sum_of_parts, diff)
    ...
    def test_measurement_noise(self):
        self.__noise_test(
            "measurement_noise", "measurement_noise_dist", MockProgModel.outputs
        )
    ...
    def test_sim_to_thresh(self):
        ...
        (times, inputs, states, outputs, event_states) = m.simulate_to_threshold(
            load, {"o1": 0.8}, dt=0.5, save_freq=1.0, x=x0
        )
        ...
        # Any event, manual
        (times, inputs, states, outputs, event_states) = m.simulate_to_threshold(
            load, {"o1": 0.8}, dt=0.5, save_freq=1.0, events=["e1", "e2"]
        )
        self.assertAlmostEqual(times[-1], 5.0, 5)
        ...
        with self.assertRaises(ValueError):
            (times, inputs, states, outputs, event_states) = m.simulate_to_threshold(
                ...
                save_freq=1.0,
                events=["e1", "e2"],
                event_strategy="fljsdk",
            )

        # Only event 2
        (times, inputs, states, outputs, event_states) = m.simulate_to_threshold(
            load, {"o1": 0.8}, dt=0.5, save_freq=1.0, events=["e2"]
        )
        self.assertAlmostEqual(times[-1], 15.0, 5)
        ...
        with self.assertWarns(Warning):
            (times, inputs, states, outputs, event_states) = m.simulate_to_threshold(
                load, {"o1": 0.8}, dt=0.5, save_freq=1.0, threshold_keys=["e2"]
            )
        self.assertAlmostEqual(times[-1], 15.0, 5)

        # Threshold before event
        (times, inputs, states, outputs, event_states) = m.simulate_to_threshold(
            load, {"o1": 0.8}, dt=0.5, save_freq=1.0, horizon=5.0, events="e2"
        )
        self.assertAlmostEqual(times[-1], 5.0, 5)

        # Threshold after event
        (times, inputs, states, outputs, event_states) = m.simulate_to_threshold(
            load, {"o1": 0.8}, dt=0.5, save_freq=1.0, horizon=20.0, events=["e2"]
        )
        self.assertAlmostEqual(times[-1], 15.0, 5)

        # No thresholds
        (times, inputs, states, outputs, event_states) = m.simulate_to_threshold(
            load, {"o1": 0.8}, dt=0.5, save_freq=1.0, horizon=20.0, events=[]
        )
        ...
        with self.assertRaises(ValueError):
            (times, inputs, states, outputs, event_states) = m.simulate_to_threshold(
                load, {"o1": 0.8}, dt=0.5, save_freq=1.0, events=[]
            )
        ...
        with self.assertRaises(ValueError):
            (times, inputs, states, outputs, event_states) = (
                m_noevents.simulate_to_threshold(
                    load, {"o1": 0.8}, dt=0.5, save_freq=1.0
                )
            )
        ...
        config = {
            ...
            "save_freq": 1.0,
            "horizon": 20.0,
            "thresholds_met_eqn": thresh_met,
        }
        with self.assertWarns(Warning):
            (times, inputs, states, outputs, event_states) = m.simulate_to_threshold(
                load, {"o1": 0.8}, **config, events=["e1", "e2"]
            )
        self.assertAlmostEqual(times[-1], 15.0, 5)
        ...
        with self.assertRaises(ValueError):
            result = m.simulate_to_threshold(
                load, {"o1": 0.8}, events=["e1", "e2", "e3"], dt=0.5, save_freq=1.0
            )
    ...
    def test_sim_namedtuple_access(self):
        ...
        z = {"o1": 0.8}
        (times, inputs, states, outputs, event_states) = m.simulate_to(
            6, load, z, dt=0.5, save_freq=1.0
        )
    ...
    def test_next_time_fcn(self):
        ...
        # With next_time
        result = m.simulate_to_threshold(
            load, {"o1": 0.8}, save_freq=1e-99, dt=next_time
        )
    ...
    def test_sim_measurement_noise(self):
        ...
        # Simulate
        (times, inputs, states, outputs, event_states) = m.simulate_to(
            3.5, load, {"o1": 0.8}, dt=0.5, save_freq=1.0
        )
        ...
        self.assertAlmostEqual(
            times[-1], 3.5, 5
        )  # Save last step (even though it's not on a savepoint)
    ...
    def test_sim_prog(self):
        ...
        (times, inputs, states, outputs, event_states) = m.simulate_to(
            3, load, {"o1": 0.8}, dt=0.5, save_freq=1.0
        )
        for t in range(0, 4):
            self.assertAlmostEqual(times[t], t, 5)
        self.assertEqual(
            len(times), 4, "Should be 4 elements in times"
        )  # Didn't save last state (because same as savepoint)

        # Check dt > save_freq
        (times, inputs, states, outputs, event_states) = m.simulate_to(
            3, load, {"o1": 0.8}, dt=0.5, save_freq=0.1
        )
        for t in range(0, 7):
            self.assertAlmostEqual(times[t], t / 2, 5)
        self.assertEqual(
            len(times), 7, "Should be 7 elements in times"
        )  # Didn't save last state (because same as savepoint)

        # Custom Savepoint test - with last state saving
        (times, inputs, states, outputs, event_states) = m.simulate_to(
            3, load, {"o1": 0.8}, dt=0.5, save_freq=99.0, save_pts=[1.45, 2.45]
        )
        # Check times
        ...
        (times, inputs, states, outputs, event_states) = m.simulate_to(
            2.5, load, {"o1": 0.8}, dt=0.5, save_freq=99.0, save_pts=[1.45, 2.45]
        )
        # Check times
        self.assertAlmostEqual(times[0], 0, 5)
        self.assertAlmostEqual(times[1], 1.5, 5)
    ...
    def test_sim_prog_inproper_config(self):
        ...
        with self.assertRaises(ValueError):
            result = m.simulate_to_threshold(
                load, {"o1": 0.8}, thresholds_met_eqn=t_met
            )
    ...
    def test_sim_modes(self):
        ...
        # Auto step size
        result = m.simulate_to_threshold(
            load, dt="auto", save_freq=0.75, save_pts=[1.5, 2.5]
        )
        self.assertListEqual(result.times, [0, 0.75, 1.5, 2.25, 2.5, 3, 3.75])

        # Auto step size with a max of 2
        result = m.simulate_to_threshold(
            load, dt=("auto", 2), save_freq=0.75, save_pts=[1.5, 2.5]
        )
        self.assertListEqual(result.times, [0, 0.75, 1.5, 2.25, 2.5, 3, 3.75])

        # Constant step size of 2
        result = m.simulate_to_threshold(
            load, dt=("constant", 2), save_freq=0.75, save_pts=[1.5, 2.5]
        )
        self.assertListEqual(result.times, [0, 2, 4])

        # Constant step size of 2
        result = m.simulate_to_threshold(
            load, dt=2, save_freq=0.75, save_pts=[1.5, 2.5]
        )
        self.assertListEqual(result.times, [0, 2, 4])
    ...
    def test_progress_bar(self):
        ...
        simulate_results = m.simulate_to_threshold(
            load, {"o1": 0.8}, dt=0.5, save_freq=1.0, print=False, progress=True
        )
        ...
        for i in range(len(capture_split)):
            actual = "%s |%s| %s%% %s" % (
                "Progress",
                "█" * percentage_vals[i] + "-" * (100 - percentage_vals[i]),
                str(percentage_vals[i]) + ".0",
                "",
            )
    ...
    def test_thrown_object_drag(self):
        ...
        simulated_results_nd = m_nd.simulate_to_threshold(
            future_load, events=[event], dt=0.005, save_freq=1
        )
        # Create default drag model ('cd' = 0.007)
        m_df = ThrownObject(process_noise_dist="none")
        simulated_results_df = m_df.simulate_to_threshold(
            future_load, events=[event], dt=0.005, save_freq=1
        )
        # Create high drag model ('cd' = 1.0)
        m_hi = ThrownObject(process_noise_dist="none")
        m_hi.parameters["cd"] = 1
        simulated_results_hi = m_hi.simulate_to_threshold(
            future_load, events=[event], dt=0.005, save_freq=1
        )
        ...
        self.assertGreater(
            simulated_results_nd.times[-1], simulated_results_df.times[-1]
        )
    ...
    def test_composite_broken(self):
        ...
        with self.assertRaises(ValueError):
            # broken in
            CompositeModel(
                [m1, m1], connections=[("z1", "OneInputOneOutputNoEventLM.u1")]
            )
        with self.assertRaises(ValueError):
            # broken out
            CompositeModel(
                [m1, m1], connections=[("OneInputOneOutputNoEventLM.z1", "u1")]
            )
        with self.assertRaises(ValueError):
            # Switched
            CompositeModel(
                [m1, m1],
                connections=[
                    ("OneInputOneOutputNoEventLM.u1", "OneInputOneOutputNoEventLM_2.z1")
                ],
            )
        with self.assertRaises(ValueError):
            ...
            CompositeModel(
                ...
                connections=[
                    (
                        "OneInputOneOutputNoEventLM.z1",
                        "OneInputOneOutputNoEventLM.u1",
                        "Something else",
                    )
                ],
            )
        ...
        with self.assertRaises(ValueError):
            ...
            CompositeModel(
                ...
                outputs=[
                    "OneInputOneOutputNoEventLM.z1",
                    "OneInputOneOutputNoEventLM_2.z1",
                    "z1",
                ],
            )
    ...
    def test_composite(self):
        ...
        self.assertSetEqual(
            m_composite.states,
            {"OneInputOneOutputNoEventLM_2.x1", "OneInputOneOutputNoEventLM.x1"},
        )
        self.assertSetEqual(
            m_composite.inputs,
            {"OneInputOneOutputNoEventLM.u1", "OneInputOneOutputNoEventLM_2.u1"},
        )
        self.assertSetEqual(
            m_composite.outputs,
            {"OneInputOneOutputNoEventLM.z1", "OneInputOneOutputNoEventLM_2.z1"},
        )
        self.assertSetEqual(m_composite.events, set())
        self.assertSetEqual(
            m_composite.performance_metric_keys,
            set(),
            "Shouldn't have any performance metrics",
        )
        ...
        u = m_composite.InputContainer(
            {"OneInputOneOutputNoEventLM.u1": 1, "OneInputOneOutputNoEventLM_2.u1": 0}
        )
        ...
        self.assertSetEqual(
            set(z.keys()),
            {"OneInputOneOutputNoEventLM_2.z1", "OneInputOneOutputNoEventLM.z1"},
        )
        ...
        self.assertSetEqual(
            m_composite.performance_metric_keys,
            {
                "OneInputOneOutputNoEventLMPM_2.x1+1",
                "OneInputOneOutputNoEventLMPM.x1+1",
            },
        )
        ...
        u = m_composite.InputContainer(
            {
                "OneInputOneOutputNoEventLMPM.u1": 1,
                "OneInputOneOutputNoEventLMPM_2.u1": 0,
            }
        )
        x = m_composite.next_state(x0, u, 1)
        pm = m_composite.performance_metrics(x)
        ...
        self.assertSetEqual(
            m_composite.states,
            {
                "OneInputOneOutputNoEventLM_2.x1",
                "OneInputOneOutputNoEventLM.x1",
                "OneInputOneOutputNoEventLM.z1",
            },
        )
        ...
        self.assertEqual(
            x["OneInputOneOutputNoEventLM_2.x1"], 1
        )  # Propagates through, because of the order. If the connection were the other way it wouldn't
        self.assertEqual(x["OneInputOneOutputNoEventLM.x1"], 1)
        z = m_composite.output(x)
        self.assertSetEqual(
            set(z.keys()),
        ...
        m_composite = CompositeModel(
            [m1, m1],
            connections=[
                ("OneInputOneOutputNoEventLM.x1", "OneInputOneOutputNoEventLM_2.u1")
            ],
        )
        ...
        m_composite = CompositeModel(
            [m2, m2],
            connections=[
                ("OneInputNoOutputOneEventLM.x1", "OneInputNoOutputOneEventLM_2.u1")
            ],
        )
        self.assertSetEqual(
            m_composite.states,
            {"OneInputNoOutputOneEventLM_2.x1", "OneInputNoOutputOneEventLM.x1"},
        )
        # One less input - since it's internally connected
        self.assertSetEqual(
            m_composite.inputs,
            {
                "OneInputNoOutputOneEventLM.u1",
            },
        )
        ...
        self.assertSetEqual(
            m_composite.events,
            {
                "OneInputNoOutputOneEventLM.x1 == 10",
                "OneInputNoOutputOneEventLM_2.x1 == 10",
            },
        )
        ...
        m_composite = CompositeModel(
            ...
            outputs=["OneInputOneOutputNoEventLM_2.z1"],
        )
        self.assertSetEqual(
            m_composite.states,
            {"OneInputOneOutputNoEventLM_2.x1", "OneInputOneOutputNoEventLM.x1"},
        ...
        # With Names
        m_composite = CompositeModel(
            [("m1", m1), ("m2", m2)], connections=[("m1.x1", "m2.u1")]
        )
    ...
    def test_parameter_equality(self):
        ...
        )  # Checking to see if the parameters are equal
        ...
        m4 = (
            LinearThrownObjectDiffDefaultParams()
        )  # Model with an extra default parameter.
        ...
        m5 = (
            LinearThrownObjectUpdatedInitializedMethod()
        )  # Model with incorrectly initialized throwing height, but same parameters
        ...
        self.assertTrue(
            m1.parameters == m2.parameters
        )  # Checking to see previous equal statements stay the same

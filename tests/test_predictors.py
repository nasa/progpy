...
class TestPredictors(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls._s = cls._m.generate_surrogate(
            [future_loading], state_keys=["v"], dt=0.1, save_freq=0.1, events="impact"
        )
    ...
    def test_UTP_Broken(self):
        ...
        samples = MultivariateNormalDist(
            ["x", "v"], [1.83, 40], [[0.1, 0.01], [0.01, 0.1]]
        )

        with self.assertRaises(ValueError):
            # Invalid event strategy - first (not supported)
            pred.predict(
                samples, dt=0.2, num_samples=3, save_freq=1, event_strategy="first"
            )
    ...
    def test_UTP_ThrownObject(self):
        ...
        results = pred.predict(
            samples, dt=0.01, save_freq=1, events=["falling", "impact"]
        )
        self.assertAlmostEqual(results.time_of_event.mean["impact"], 8.21, 0)
        self.assertAlmostEqual(results.time_of_event.mean["falling"], 4.15, 0)
        ...
        with self.assertRaises(ValueError):
            # Mix valid, invalid
            results = pred.predict(
                samples, dt=0.01, save_freq=1, events=["falling", "invalid"]
            )
        with self.assertRaises(ValueError):
            # Empty
    ...
    def test_UTP_ThrownObject_One_Event(self):
        ...
        results = pred.predict(
            samples, future_loading, dt=0.01, events=["falling"], save_freq=1
        )
        ...
        self.assertAlmostEqual(
            results.times[-1], 3, 1
        )  # Saving every second, last time should be around the nearest 1s before falling event
    ...
    def test_MC_ThrownObject(self):
        ...
        results = mc.predict(
            m.initialize(), dt=0.2, num_samples=3, save_freq=1, event_strategy="all"
        )
        self.assertAlmostEqual(results.time_of_event.mean["impact"], 8.0, 5)
        self.assertAlmostEqual(results.time_of_event.mean["falling"], 3.8, 5)
        ...
        # Setting event manually
        results = mc.predict(
            m.initialize(), dt=0.2, num_samples=3, save_freq=1, events=["falling"]
        )
        self.assertAlmostEqual(results.time_of_event.mean["falling"], 3.8, 5)
        self.assertNotIn("impact", results.time_of_event.mean)
        ...
        # String event
        results = mc.predict(
            m.initialize(), dt=0.2, num_samples=3, save_freq=1, events="impact"
        )
        self.assertAlmostEqual(results.time_of_event.mean["impact"], 8.0, 5)
        self.assertNotIn("falling", results.time_of_event.mean)
        ...
        with self.assertRaises(ValueError):
            results = mc.predict(
                m.initialize(), dt=0.2, num_samples=3, save_freq=1, events="invalid"
            )
        with self.assertRaises(ValueError):
            # Mix valid, invalid
            results = mc.predict(
                ...
                num_samples=3,
                save_freq=1,
                events=["falling", "invalid"],
            )
        with self.assertRaises(ValueError):
            # Empty
            results = mc.predict(
                m.initialize(), dt=0.2, num_samples=3, save_freq=1, events=[]
            )
        with self.assertRaises(TypeError):
            results = mc.predict(
                m.initialize(), dt=0.2, num_samples=3, save_freq=1, events=45
            )

        # Empty with horizon
        results = mc.predict(
            m.initialize(), dt=0.2, num_samples=3, save_freq=1, horizon=3, events=[]
        )
    ...
    def test_MC_ThrownObject_First(self):
        ...
        mc_results = mc.predict(
            m.initialize(), dt=0.2, event_strategy="first", num_samples=3, save_freq=1
        )
    ...
    def test_prediction_mvnormaldist(self):
        ...
        states = [
            MultivariateNormalDist(means[i].keys(), means[i].values(), covar)
            for i in range(10)
        ]
    ...
    def test_prediction_uwsamples(self):
        times = list(range(10))
        states = [
            UnweightedSamples([{"a": i} for i in range(10)]),
        ...
        self.assertEqual(
            p.snapshot(0), UnweightedSamples([{"a": 0}, {"a": 1}, {"a": -1}])
        )
        self.assertEqual(
            p.snapshot(-1), UnweightedSamples([{"a": 9}, {"a": 10}, {"a": 8}])
        )
        self.assertEqual(p.times[0], times[0])
        self.assertEqual(p.times[-1], times[-1])
    ...
    def test_pickle_prediction_uwsamples(self):
        times = list(range(10))
        states = [
            UnweightedSamples(list(range(10))),
    ...
    def test_profile_plot(self):
        ...
        sample_alpha = 0.50
        gt_and_alpha_plots = profile.plot(
            ground_truth=sample_gt, alpha=sample_alpha, show=True
        )
    ...
    def test_prediction_monotonicity(self):
        ...
        self.assertDictEqual(
            p.monotonicity(), {"a": 0.2222222222222222, "b": 0.2222222222222222}
        )
        ...
        p = Prediction(times, states)
        self.assertDictEqual(
            p.monotonicity(), {"a": 1, "b": 1, "c": 0, "d": 0.2222222222222222}
        )
    ...
    def _test_surrogate_pred(self, Predictor, **kwargs):
        ...
        x0 = s.initialize()
        x0 = MultivariateNormalDist(
            x0.keys(), x0.values(), np.diag([1e-8 * xi for xi in x0.values()])
        )
        result = p.predict(x0, future_loading, horizon=50, dt=0.1)

    def test_utp_surrogate(self):
        self._test_surrogate_pred(
            UnscentedTransformPredictor, Q=np.diag([1e-3, 1e-3, 1e-7, 1e-7])
        )
    ...
    def test_mc_num_samples(self):
        ...
        x_uwsamples = UnweightedSamples(
            [{"x": 10, "v": 0}, {"x": 9.9, "v": 0.1}, {"x": 10.1, "v": -0.1}]
        )

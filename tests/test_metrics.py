...
class TestMetrics(unittest.TestCase):
        ...
        def check_metrics(metrics):
            ...
            for key in keys:
                ...
                self.assertAlmostEqual(
                    metrics[key]["percentiles"]["50"], mean_dict[key]
                )
                self.assertAlmostEqual(metrics[key]["std"], 1, 1)

        mean = [10, 11, 12]
        keys = ["a", "b", "c"]
        covar = [[1, 0.1, 0.1], [0.1, 1, 0.1], [0.1, 0.1, 1]]
    ...
    def test_toe_metrics_scalar(self):
        ...
        for key in scalar.keys():
            ...
            metrics[key]["percentiles"]["0.01"] = data[
                key
            ]  # Fill so we can check everything else below
        check_metrics(metrics)
    ...
    def test_toe_profile_metrics(self):
        ...
        for i in range(10):
            ...
            data = [
                {"a": j, "b": j - 1, "c": (j - 4.5) * 2 + 4.5} for j in range(i, i + 20)
            ]
        ...
        self.assertDictEqual(
            metrics, profile.alpha_lambda(ground_truth, lambda_value, alpha, beta)
        )

        # Now lets try specifying only keys a and b
        metrics = alpha_lambda(
            profile, ground_truth, lambda_value, alpha, beta, keys=["a", "b"]
        )
        self.assertIn("a", metrics)
        self.assertIn("b", metrics)
        self.assertNotIn("c", metrics)
    ...
    def test_toe_profile_prognostic_horizon(self):
        ...
        self.assertDictEqual(
            profile.prognostic_horizon(criteria_eqn, GROUND_TRUTH),
        ...
        GROUND_TRUTH = {
            ...
            "b": 10.0,
            "c": 18.0,
        }  # Adjust ground truth to match 3 criteria
        self.assertDictEqual(
            profile.prognostic_horizon(criteria_eqn, GROUND_TRUTH),
            {"a": 1.0, "b": 2.0, "c": 10.0},
        )
    ...
    def test_toe_profile_cumulative_relative_accuracy(self):
        ...
        self.assertEqual(
            ...
            {"a": 0.4444444444444445, "b": 0.375, "c": 0.6388888888888888},
        )
        # Test negative floats ground truth
        GROUND_TRUTH = {"a": -9.0, "b": -8.0, "c": -18.0}
        self.assertEqual(
            profile.cumulative_relative_accuracy(GROUND_TRUTH),
            {"a": 3.555555555555556, "b": 3.625, "c": 3.305555555555556},
        )
    ...
    def test_toe_profile_monotonicity(self):
        ...
        for i in range(11):
            ...
            profile.add_prediction(
                i,  # Time (reverse so data is decreasing)
                MultivariateNormalDist(
                    data[i].keys(), data[i].values(), covar
                ),  # ToE Prediction
            )

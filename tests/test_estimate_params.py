...
class TestEstimateParams(unittest.TestCase):
    ...
    def test_estimate_params(self):
        ...
        self.assertEqual(
            "Missing keyword arguments times, inputs, outputs", str(cm.exception)
        )
        ...
        self.assertEqual("Missing keyword arguments inputs, outputs", str(cm.exception))

        with self.assertRaises(ValueError) as cm:
            m.estimate_params(times=results.times, inputs=results.inputs, outputs=None)
        self.assertEqual("Missing keyword arguments outputs", str(cm.exception))

        with self.assertRaises(ValueError) as cm:
            m.estimate_params(times=None, inputs=[None], outputs=[])
        self.assertEqual("Missing keyword arguments times", str(cm.exception))
        ...
        m.estimate_params(
            ...
            outputs=results.outputs,
            keys=keys,
            bounds={"thrower_height": (0, 4), "throwing_speed": (20, 42)},
        )
        for key in keys:
            self.assertAlmostEqual(m.parameters[key], gt[key], 2)
        ...
        with self.assertRaises(TypeError):
            m.estimate_params(
                times=[results.inputs],
                inputs=[results.times],
                outputs=[results.outputs],
                keys=keys,
            )
        ...
        with self.assertRaises(ValueError):
            m.estimate_params(
                times=[results.times], outputs=[results.outputs], keys=keys
            )
        ...
        m.parameters["throwing_speed"] = 25
        m.estimate_params(
            times=results.times, inputs=results.inputs, outputs=results.outputs
        )
        ...
        m.estimate_params(
            ...
            outputs=results.outputs,
            keys=keys,
            bounds=np.array([[0, 4], [20, 42], [-12, 15]]),
        )
        for key in keys:
            self.assertAlmostEqual(m.parameters[key], gt[key], 2)
        ...
        m.estimate_params(
            ...
            outputs=results.outputs,
            keys=keys,
            bounds=np.array([(0, 4), (-1000, 42), (-900, 0)]),
        )
        for key in keys:
            self.assertAlmostEqual(m.parameters[key], gt[key], 2)
        ...
        with self.assertRaises(ValueError):
            m.estimate_params(
                ...
                outputs=results.outputs,
                keys=keys,
                bounds=np.array([(1, 2), (2, 3), (4, 5), (-1, 20)]),
            )
        ...
        m.estimate_params(
            ...
            outputs=results.outputs,
            keys=keys,
            bound=np.array([(False, True), (False, True), (False, True)]),
        )
        ...
        m.estimate_params(
            ...
            outputs=results.outputs,
            keys=keys,
            bounds=[np.array([1, 2]), np.array([2, 3]), np.array([4, 5])],
        )
        ...
        with self.assertRaises(ValueError):
            m.estimate_params(
                ...
                outputs=results.outputs,
                keys=keys,
                bounds=[np.array([1, 2]), np.array([2, 3])],
            )
        ...
        m.estimate_params(
            ...
            outputs=results.outputs,
            keys=keys,
            bounds=[np.array(["4", "9"]), np.array(["2", "3"]), np.array(["4", "5"])],
        )
        ...
        m.estimate_params(
            ...
            outputs=results.outputs,
            keys=keys,
            bounds=[np.array(["4", "9"]), np.array([2, 3]), np.array([4.123, 5.346])],
        )
        ...
        with self.assertRaises(ValueError):
            m.estimate_params(
                ...
                outputs=results.outputs,
                keys=keys,
                bounds=np.array([(True, False), (False, True), (False, True)]),
            )
        ...
        m.estimate_params(
            ...
            outputs=results.outputs,
            keys=keys,
            bounds=([-3, 12], (1, 400), np.array([-20, 30])),
        )
        for key in keys:
            self.assertAlmostEqual(m.parameters[key], gt[key], 2)
        ...
        m.estimate_params(
            ...
            outputs=results.outputs,
            keys=keys,
            bounds=np.array([("-3", "12"), ("1", "42"), ("-12", "30")]),
        )
        for key in keys:
            self.assertAlmostEqual(m.parameters[key], gt[key], 2)
        ...
        m.estimate_params(
            ...
            outputs=results.outputs,
            keys=keys,
            bounds=(("-3.12", "12"), ("1", "42.125381"), ("-12", "30")),
        )
        ...
        with self.assertRaises(ValueError):
            m.estimate_params(
                ...
                outputs=results.outputs,
                keys=keys,
                bounds=[np.array(["a", "s"]), np.array([2, 3]), np.array([4, 5])],
            )
        ...
        m.estimate_params(
            ...
            outputs=results.outputs,
            keys=keys,
            bounds=[np.array(["1", 9]), np.array([2, 3]), np.array([4, 5])],
        )
        ...
        m.estimate_params(
            ...
            outputs=results.outputs,
            keys=keys,
            bounds=[np.array(["9", "9"]), np.array(["2", "3"]), np.array(["4", "5"])],
        )
        ...
        self.assertFalse(
            np.array_equal(m_result["final_simplex"], m2_result["final_simplex"])
        )
    ...
    def test_parameters(self):
        ...
        with self.assertRaises(TypeError):
            m.estimate_params(
                ...
                bounds=bound,
                method="Powell",
                options={"maxiter": "9999", "disp": False},
            )
    ...
    def test_multiple_runs(self):
        ...
        with self.assertRaises(ValueError) as cm:
            m.estimate_params(
                times=incorrectTimesRunsLen, inputs=inputs, outputs=outputs
            )
        self.assertEqual(
            "Times, inputs, and outputs must be same length. Length of times: 1, Length of inputs: 2, Length of outputs: 2",
            str(cm.exception),
    ...
    def test_incorrect_lens(self):
        ...
        with self.assertRaises(ValueError):
            m.estimate_params(
                times=[results.times], inputs=[wrongInputLen], outputs=[results.outputs]
            )
        ...
        with self.assertRaises(ValueError) as cm:
            m.estimate_params(
                times=[results.times], inputs=[results.inputs], outputs=[wrongOutputs]
            )
        self.assertEqual(
        ...
        with self.assertRaises(ValueError) as cm:
            m.estimate_params(
                times=[results.times], inputs=[wrongInputLen], outputs=[wrongOutputs]
            )
        self.assertEqual(
        ...
        self.assertEqual("Missing keyword arguments inputs", str(cm.exception))

        # 'input' is not a keyword argument, so technically not defining the inputs.
        with self.assertRaises(ValueError) as cm:
            m.estimate_params(
                times=[results.times], input=[results.inputs], outputs=[results.outputs]
            )
        self.assertEqual("Missing keyword arguments inputs", str(cm.exception))
    ...
    def test_tolerance(self):
        ...
        if not all(abs(m.parameters[key] - gt[key]) > 0.02 for key in keys):
            raise ValueError(
                "m.parameter shouldn't be too close to the original parameters"
            )
...
def main():
    ...
    result = runner.run(
        load_test.loadTestsFromTestCase(TestEstimateParams)
    ).wasSuccessful()

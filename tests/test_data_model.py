...
from progpy.data_models import LSTMStateTransitionModel, DataModel, DMDModel
...
class TestDataModel(unittest.TestCase):
    ...
    def _test_simple_case(
        ...
        **kwargs,
        ...
        data = m.simulate_to_threshold(
            future_loading, events="impact", save_freq=TIMESTEP, dt=TIMESTEP
        )
    ...
    def test_early_stopping(self):
        ...
        cfg = {"dt": 0.1, "save_freq": 0.1, "window": 4, "epochs": 75}

        # No early stopping
        _stdout = sys.stdout
        sys.stdout = StringIO()
        m2 = LSTMStateTransitionModel.from_model(
            m, [future_loading], early_stop=False, **cfg
        )
    ...
    def test_lstm_simple(self):
        m = self._test_simple_case(
            LSTMStateTransitionModel, window=5, epochs=20, max_error=3
        )
        ...
        LSTMStateTransitionModel(
            m.parameters["output_model"], m.parameters["state_model"], output_keys=["x"]
        )
        try:
            ...
            warnings.warn(
                "Pickling not supported for LSTMStateTransitionModel on this system"
            )
            pass
    ...
    def test_lstm_from_model_thrown_object(self):
        ...
        def future_loading2(t, x=None):
            ...
            nonlocal t_counter, x_counter
            z = m.output(x_counter)
            z = m3.InputContainer({"x_t-1": z["x"], "dt": TIMESTEP})
            x_counter = m.next_state(x_counter, future_loading(t), t - t_counter)
            t_counter = t
            return z
...
def main():
    ...
    with patch("matplotlib.pyplot.show"):
        result = runner.run(
            load_test.loadTestsFromTestCase(TestDataModel)
        ).wasSuccessful()
    plt.close("all")

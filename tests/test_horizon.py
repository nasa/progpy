...
class TestHorizon(unittest.TestCase):
    ...
    def test_horizon_ex(self):
        ...
        mc_results = mc.predict(
            initial_state, future_loading, dt=STEP_SIZE, horizon=PREDICTION_HORIZON
        )

        # 'falling' happens before the horizon is met
        falling_res = [
            mc_results.time_of_event[iter]["falling"]
            for iter in range(mc.parameters["n_samples"])
            if mc_results.time_of_event[iter]["falling"] is not None
        ]
        ...
        impact_res = [
            mc_results.time_of_event[iter]["impact"]
            for iter in range(mc.parameters["n_samples"])
            if mc_results.time_of_event[iter]["impact"] is not None
        ]
        ...
        mc_results_no_event = mc.predict(
            initial_state, future_loading, dt=STEP_SIZE, horizon=0.3
        )
        falling_res_no_event = [
            mc_results_no_event.time_of_event[iter]["falling"]
            for iter in range(mc.parameters["n_samples"])
            if mc_results_no_event.time_of_event[iter]["falling"] is None
        ]
        impact_res_no_event = [
            mc_results_no_event.time_of_event[iter]["impact"]
            for iter in range(mc.parameters["n_samples"])
            if mc_results_no_event.time_of_event[iter]["impact"] is None
        ]
        ...
        falling_res_all_event = [
            mc_results_all_event.time_of_event[iter]["falling"]
            for iter in range(mc.parameters["n_samples"])
            if mc_results_all_event.time_of_event[iter]["falling"] is not None
        ]
        impact_res_all_event = [
            mc_results_all_event.time_of_event[iter]["impact"]
            for iter in range(mc.parameters["n_samples"])
            if mc_results_all_event.time_of_event[iter]["impact"] is not None
        ]

...
class TestUncertainData(unittest.TestCase):
    def test_unweightedsamples(self):
        ...
        self.assertEqual(data.percentage_in_bounds([0, 2.5]), {"a": 0.6, "b": 0.4})
        self.assertEqual(
            data.percentage_in_bounds({"a": [0, 2.5], "b": [0, 1.5]}),
            {"a": 0.6, "b": 0.2},
        )
    ...
    def test_relative_accuracy(self):
        ...
        with self.assertRaises(
            ZeroDivisionError
        ):  # Passing in ground truth of 0 leads to divide by 0 error
            gt_zero = {"a": 0, "b": 0}
        ...
        with self.assertRaises(
            ZeroDivisionError
        ):  # Hits -inf and nan; maybe because 0/0?
            gt_zero = {"a": 0, "b": 0}
            ra_zero = d.relative_accuracy(gt_zero)
        with self.assertRaises(TypeError):  # Passing in non-dict arg
            ra_err_list = d.relative_accuracy([])
        ...
        self.assertDictEqual(
            ra_std, {"a": 0.6666666666666667, "b": -1.3333333333333335}
        )

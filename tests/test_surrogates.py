...
from progpy.models import ThrownObject, BatteryElectroChemEOD, DCMotorSP
from progpy.models.test_models.linear_models import (
    OneInputNoOutputOneEventLM,
    OneInputNoOutputTwoEventLM,
    TwoInputNoOutputOneEventLM,
    TwoInputNoOutputTwoEventLM,
)
...
class TestSurrogate(unittest.TestCase):
    ...
    def test_surrogate_improper_input(self):
        ...
        with self.assertRaises(ValueError):
            m.generate_surrogate([load_eqn], state_keys=["invalid"])
        with self.assertRaises(ValueError):
            m.generate_surrogate([load_eqn], state_keys=["x", "invalid", "v"])
        with self.assertRaises(ValueError):
            m.generate_surrogate([load_eqn], input_keys=["invalid"])
        with self.assertRaises(ValueError):
            m.generate_surrogate([load_eqn], output_keys=["invalid"])
        with self.assertRaises(ValueError):
            m.generate_surrogate([load_eqn], output_keys=["invalid", "x"])
        with self.assertRaises(ValueError):
            m.generate_surrogate([load_eqn], event_keys=["invalid"])
        with self.assertRaises(ValueError):
            m.generate_surrogate(
                [load_eqn], event_keys=["falling", "impact", "invalid"]
            )
        with self.assertRaises(ValueError):
    ...
    def test_surrogate_basic_thrown_object(self):
        ...
        surrogate = m.generate_surrogate(
            [load_eqn], dt=0.1, save_freq=0.25, events="impact", training_noise=0
        )
        ...
        self.assertListEqual(
            ...
            [
                stateTest
                for stateTest in m.states
                if (stateTest not in m.outputs and stateTest not in m.events)
            ]
            + m.outputs
            + m.events,
        )
        ...
        options = {"events": "impact", "save_freq": 0.25, "dt": 0.25}
        ...
        MSE = m.calc_error(
            surrogate_results.times, surrogate_results.inputs, surrogate_results.outputs
        )
        self.assertLess(MSE, 10)  # Pretty good approx

        self.assertAlmostEqual(
            surrogate_results.times[-1], result.times[-1], delta=0.26
        )
        for i in range(min(len(result.times), len(surrogate_results.times))):
            self.assertListEqual(
                list(surrogate_results.inputs[i].keys()),
                list(surrogate_results.inputs[i].keys()),
            )
            self.assertAlmostEqual(
                surrogate_results.states[i]["x"], result.states[i]["x"], delta=8
            )
            self.assertEqual(
                surrogate_results.states[i]["x"], surrogate_results.outputs[i]["x"]
            )
            self.assertAlmostEqual(
                surrogate_results.states[i]["v"], result.states[i]["v"], delta=1
            )
            self.assertAlmostEqual(
                surrogate_results.states[i]["falling"],
                result.event_states[i]["falling"],
                delta=0.1,
            )
            self.assertEqual(
                surrogate_results.states[i]["falling"],
                surrogate_results.event_states[i]["falling"],
            )
            self.assertAlmostEqual(
                surrogate_results.states[i]["impact"],
                result.event_states[i]["impact"],
                delta=0.1,
            )
            self.assertEqual(
                surrogate_results.states[i]["impact"],
                surrogate_results.event_states[i]["impact"],
            )
    ...
    def test_surrogate_basic_battery(self):
        ...
        self.assertSetEqual(
            set(surrogate.states),
            set(["tb", "Vsn", "Vsp"] + options_surrogate["output_keys"] + m.events),
        )
        self.assertListEqual(surrogate.inputs, m.inputs)
        ...
        surrogate_results = surrogate.simulate_to_threshold(
            future_loading, **options_sim
        )
    ...
    def test_surrogate_subsets(self):
        ...
        self.assertListEqual(
            ...
            [
                ...
                for stateTest in m.states
                if (
                    stateTest not in m.inputs
                    and stateTest not in m.outputs
                    and stateTest not in m.events
                )
            ]
            + m.outputs
            + m.events,
        ...
        for i in range(min(len(result.times), len(surrogate_results.times))):
            ...
            self.assertAlmostEqual(
                surrogate_results.states[i]["v"], result.states[i]["v"], delta=1.25
            )
            self.assertAlmostEqual(
                surrogate_results.states[i]["impact"],
                result.event_states[i]["impact"],
    ...
    def test_surrogate_thrown_object_with_noise(self):
        ...
        surrogate_noise = m.generate_surrogate(
            [load_eqn], dt=0.1, save_freq=0.25, events="impact", training_noise=0.01
        )
        self.assertEqual(surrogate.dt, 0.25)
        ...
        surrogate_results = surrogate.simulate_to_threshold(load_eqn, **options)
        surrogate_noise_results = surrogate_noise.simulate_to_threshold(
            load_eqn, **options
        )

        position = [
            surrogate_results.states[iter]["x"]
            for iter in range(len(surrogate_results.times))
        ]
        position_noise = [
            surrogate_noise_results.states[iter]["x"]
            for iter in range(len(surrogate_noise_results.times))
        ]

        MSE = sum(
            [
                (position[iter] - position_noise[iter]) ** 2
                for iter in range(min(len(position), len(position_noise)))
            ]
        ) / min(len(position), len(position_noise))
        self.assertLess(MSE, 10)
        self.assertNotEqual(MSE, 0)

        self.assertAlmostEqual(
            surrogate_results.times[-1], surrogate_noise_results.times[-1], delta=0.26
        )
        for i in range(
            min(len(surrogate_results.times), len(surrogate_noise_results.times))
        ):
            self.assertListEqual(
                list(surrogate_noise_results.inputs[i].keys()),
                list(surrogate_noise_results.inputs[i].keys()),
            )
            self.assertAlmostEqual(
                surrogate_noise_results.states[i]["x"],
                surrogate_results.states[i]["x"],
                delta=6,
            )
            self.assertAlmostEqual(
                surrogate_noise_results.states[i]["v"],
                surrogate_results.states[i]["v"],
                delta=1,
            )
            self.assertAlmostEqual(
                surrogate_noise_results.states[i]["falling"],
                surrogate_results.states[i]["falling"],
                delta=0.1,
            )
            self.assertAlmostEqual(
                surrogate_noise_results.states[i]["impact"],
                surrogate_results.states[i]["impact"],
                delta=0.5,
            )
            self.assertEqual(
                surrogate_noise_results.states[i]["x"],
                surrogate_noise_results.outputs[i]["x"],
            )
            self.assertEqual(
                surrogate_noise_results.states[i]["falling"],
                surrogate_noise_results.event_states[i]["falling"],
            )
            self.assertEqual(
                surrogate_noise_results.states[i]["impact"],
                surrogate_noise_results.event_states[i]["impact"],
            )
            self.assertAlmostEqual(
                surrogate_noise_results.states[i]["falling"],
                surrogate_results.event_states[i]["falling"],
    ...
    def test_surrogate_battery_with_noise(self):
        ...
        surrogate = m.generate_surrogate(load_functions, **options_surrogate)
        surrogate_noise = m.generate_surrogate(
            load_functions, **options_surrogate_noise
        )
        ...
        voltage = [
            surrogate_results.states[iter]["v"]
            for iter in range(len(surrogate_results.times))
        ]
        voltage_noise = [
            surrogate_noise_results.states[iter]["v"]
            for iter in range(len(surrogate_noise_results.times))
        ]

        MSE = sum(
            [
                (voltage[iter] - voltage_noise[iter]) ** 2
                for iter in range(min(len(voltage), len(voltage_noise)))
            ]
        ) / min(len(voltage), len(voltage_noise))
        self.assertLess(MSE, 0.02)  # Pretty good approx
        self.assertNotEqual(MSE, 0)

        self.assertAlmostEqual(
            surrogate_results.times[-1], surrogate_noise_results.times[-1], delta=20
        )
        for i in range(
            min(len(surrogate_results.times), len(surrogate_noise_results.times))
        ):
            ...
            self.assertAlmostEqual(
                surrogate_noise_results.states[i]["tb"],
                surrogate_results.states[i]["tb"],
                delta=10,
            )
            self.assertAlmostEqual(
                surrogate_noise_results.states[i]["Vo"],
                surrogate_results.states[i]["Vo"],
                delta=0.1,
            )
            self.assertAlmostEqual(
                surrogate_noise_results.states[i]["Vsn"],
                surrogate_results.states[i]["Vsn"],
                delta=0.5,
            )
            self.assertAlmostEqual(
                surrogate_noise_results.states[i]["Vsp"],
                surrogate_results.states[i]["Vsp"],
                delta=0.1,
            )
            self.assertAlmostEqual(
                surrogate_noise_results.states[i]["qnB"],
                surrogate_results.states[i]["qnB"],
                delta=55,
            )
            self.assertAlmostEqual(
                surrogate_noise_results.states[i]["qnS"],
                surrogate_results.states[i]["qnS"],
                delta=8,
            )
            self.assertAlmostEqual(
                surrogate_noise_results.states[i]["qpB"],
                surrogate_results.states[i]["qpB"],
                delta=55,
            )
            self.assertAlmostEqual(
                surrogate_noise_results.states[i]["qpS"],
                surrogate_results.states[i]["qpS"],
                delta=8,
            )
            self.assertAlmostEqual(
                surrogate_noise_results.states[i]["v"],
            ...
            self.assertAlmostEqual(
                surrogate_noise_results.states[i]["EOD"],
                surrogate_results.states[i]["EOD"],
                delta=0.1,
            )
            self.assertEqual(
                surrogate_noise_results.states[i]["v"],
                surrogate_noise_results.outputs[i]["v"],
            )
            self.assertEqual(
                surrogate_noise_results.states[i]["EOD"],
                surrogate_noise_results.event_states[i]["EOD"],
            )
            self.assertAlmostEqual(
                surrogate_noise_results.states[i]["v"],
                surrogate_results.outputs[i]["v"],
                delta=0.3,
            )
            self.assertAlmostEqual(
                surrogate_noise_results.states[i]["EOD"],
                surrogate_results.event_states[i]["EOD"],
                delta=0.1,
            )
            if i > 0:
                self.assertNotEqual(
    ...
    def test_surrogate_output_interp(self):
        ...
        self.assertEqual(
            (surrogate_results.times[1] - surrogate_results.times[0]),
            options_sim["save_freq"],
        )
    ...
    def _pce_tests(self, m):
        ...
        pce = PCE.from_model(
            m, x0, input_dists, times=[i * 10 for i in range(5)], N=250
        )
        pce_result = pce.time_of_event(
            x0, lambda t, x=None: pce.InputContainer({"u1": 1, "u2": 0.75})
        )
        gt_result = m.time_of_event(
            x0, lambda t, x=None: m.InputContainer({"u1": 1, "u2": 0.75})
        )
        ...
        input_dists = {key: input_dists[key] for key in m.inputs}
        pce = PCE.from_model(
            m, x0, input_dists, times=[i * 10 for i in range(6)], N=250
        )
        pce_result = pce.time_of_event(
            x0, lambda t, x=None: pce.InputContainer({"u1": 1, "u2": 0.75})
        )
        ...
        pce_result = pce.time_of_event(
            x0, lambda t, x=None: pce.InputContainer({"u1": 1.5, "u2": 1})
        )
        gt_result = m.time_of_event(
            x0, lambda t, x=None: m.InputContainer({"u1": 1.5, "u2": 1})
        )
        for event in m.events:

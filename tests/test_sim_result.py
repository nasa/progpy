...
class TestSimResult(unittest.TestCase):
    ...
    def test_extend(self):
        ...
        self.assertRaises(
            ValueError, result.extend, 0
        )  # Passing non-LazySimResult types to extend method
    ...
    def test_pickle_lazy(self):
        ...
        time = list(range(5))  # list of int, 0 to 4
        state = [{"a": i * 2.5, "b": i * 2.5} for i in range(5)]
        lazy_result = LazySimResult(
            f, time, state
        )  # Ordinary LazySimResult with f, time, state
        ...
        self.assertNotEqual(
            sim_result, converted_lazy_result
        )  # converted is not the same as the original SimResult
    ...
    def test_index(self):
        ...
        self.assertRaises(
            ValueError, result.index, "7.5"
        )  # Data specified incorrectly as string
        self.assertRaises(
            ValueError, result.index, None
        )  # Not type errors because its simply looking for an object in list
    ...
    def test_pop(self):
        ...
        self.assertEqual(result.data, state)
        self.assertRaises(
            IndexError, result.pop, 5
        )  # Test specifying an invalid index value
        self.assertRaises(IndexError, result.pop, 3)
        self.assertRaises(
            TypeError, result.pop, "5"
        )  # Test specifying an invalid index type
    ...
    def test_to_numpy(self):
        ...
        self.assertTrue(
            np.all(np_result == np.array([[i * 2.5, i * 5] for i in range(10)]))
        )
        ...
        self.assertEqual(result.dtype, np.dtype("float64"))
        self.assertTrue(
            np.all(result == np.array([[i * 2.5, i * 5] for i in range(10)]))
        )
    ...
    def test_remove(self):
        ...
        self.assertRaises(
            ValueError,
            result.remove,
        )  # If nothing specified, raise ValueError
        self.assertRaises(ValueError, result.remove, None, None)  # Passing both as None
        self.assertRaises(
            ValueError, result.remove, 0.0, 1
        )  # Passing arguments to both
        self.assertRaises(ValueError, result.remove, 7.5)  # Test nonexistent data value
        self.assertRaises(
            ValueError, result.remove, -1
        )  # Type checking negated as index searches for element in list
        self.assertRaises(
            ValueError, result.remove, "5"
        )  # Thus all value types allowed to be searched
    ...
    def test_get_time(self):
        ...
        self.assertRaises(
            TypeError, result.time, "0"
        )  # Tests specifying an invalid index type
    ...
    def test_plot(self):
        ...
        class MyBatt(BatteryElectroChemEOD):
            ...
            def event_state(self, state):
                ...
                event_state["EOD_warn_yellow"] = (
                    event_state["EOD"] - YELLOW_THRESH
                ) / (1 - YELLOW_THRESH)
        ...
        (times, inputs, states, outputs, event_states) = m.simulate_to_threshold(
            future_loading, events="EOD", print=False
        )
    ...
    def test_namedtuple_access(self):
        ...
        m = MyBatt()
        named_results = m.simulate_to_threshold(
            future_loading, events=["EOD"], print=False
        )
    ...
    def test_lazy_extend(self):
        ...
        self.assertEqual(
            result.times, time + time2
        )  # Assert data is correct after extending
    ...
    def test_lazy_extend_error(self):
        ...
        self.assertRaises(
            ValueError, result.extend, sim_result
        )  # Passing a SimResult to LazySimResult's extend
        self.assertRaises(
            ValueError, result.extend, 0
        )  # Passing non-LazySimResult types to extend method
        self.assertRaises(ValueError, result.extend, [0, 1])
    ...
    def test_lazy_remove(self):
        ...
        result.remove(
            {"a": 5.0, "b": 10}
        )  # Unnamed default positional argument removal of data value
        # Update Variables
        state.remove({"a": 2.5, "b": 5})
        time.remove(1)
        ...
        self.assertRaises(
            ValueError, result.remove, 90.0, 2
        )  # Test two values specified positionally
        self.assertRaises(
            ValueError, result.remove, 90.0, 2, 15.0
        )  # Test three values specified positionally
        self.assertRaises(
            ValueError, result.remove, d=90.0, t=2
        )  # Test d,t values specified by name
        self.assertRaises(
            ValueError, result.remove, t=2, s=15.0
        )  # Test s,t values specified by name
        self.assertRaises(
            ValueError, result.remove, d=90.0, s=15.0
        )  # Test d,s values specified by name
        self.assertRaises(
            ValueError, result.remove, d=90.0, t=2, s=15.0
        )  # Test three values specified by name
        self.assertRaises(
            ValueError, result.remove, 90.0
        )  # Test nonexistent data value
        self.assertRaises(
            ValueError, result.remove, d=90.0
        )  # Test nonexistent data value
        self.assertRaises(
            ValueError, result.remove, t=90.0
        )  # Test nonexistent times value
        self.assertRaises(
            ValueError, result.remove, s=90.0
        )  # Test nonexistent states value
        self.assertRaises(
            ValueError, result.remove, -1
        )  # Type checking negated as index searches for element in list
        self.assertRaises(
    ...
    def test_lazy_to_simresult(self):
        ...
        self.assertTrue(
            isinstance(converted_result, SimResult)
        )  # Ensure type is SimResult
        self.assertEqual(
            converted_result.times, result.times
        )  # Compare to original LazySimResult

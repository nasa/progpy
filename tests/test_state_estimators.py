...
def equal_cov(pair1, pair2):
    ...
    return all(
        [
            cov1[i][j] == cov2[mapping[i]][mapping[j]]
...
class MockProgModel2(MockProgModel):
    ...
    def output(self, x):
        return self.OutputContainer({"o1": x["a"] + x["b"] + x["c"], "o2": 7})
...
class TestStateEstimators(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls._s = cls._m.generate_surrogate(
            [future_loading], state_keys=["v"], dt=0.1, save_freq=0.1
        )
    ...
    def test_UKF(self):
        ...
        x_guess = {
            "x": 1.75,
            "v": 35,
        }  # Guess of initial state, actual is {'x': 1.83, 'v': 40}
        ...
        x_mvnd = MultivariateNormalDist(
            ["x", "v"], np.array([2, 10]), np.array([[1, 0], [0, 1]])
        )
        ...
        x_mvnd = MultivariateNormalDist(
            ["v", "x"], np.array([10, 2]), np.array([[1, 0], [0, 2]])
        )
        filt_mvnd = UnscentedKalmanFilter(m, x_mvnd)
        mean1 = filt_mvnd.x.mean
        mean2 = x_mvnd.mean
        ...
        self.assertTrue(
            ...
            "Covs are not equal for multivariate in different order",
        )
        ...
        self.assertTrue(
            equal_cov(
                (list(x_us.keys()), x_us.cov), (list(filt_us.x.keys()), filt_us.x.cov)
            )
        )
    ...
    def test_PF_limit_check(self):
        ...
        # With state limits
        OneInputOneOutputOneEventLM.state_limits = {"x1": (0, 10)}
    ...
    def test_PF(self):
        ...
        filt = ParticleFilter(
            m, x_guess, num_particles=1000, measurement_noise={"x": 1}
        )
        self.__test_state_est(filt, m)
        ...
        filt_scalar = ParticleFilter(
            m, x_scalar, num_particles=20
        )  # Sample count does not affect ScalarData testing
        mean1 = filt_scalar.x.mean
        mean2 = x_scalar.mean
        self.assertSetEqual(set(mean1.keys()), set(mean2.keys()))
        for k in mean1.keys():
        ...
        for i in range(len(filt_mvnd.x.cov)):
            for j in range(len(filt_mvnd.x.cov[i])):
                self.assertAlmostEqual(
                    filt_mvnd.x.cov[i][j], x_mvnd.cov[i][j], delta=0.1
                )
        ...
        for i in range(x0_samples):
            uw_input.append(
                {
                    "x": random.randrange(-x_bounds, x_bounds),
                    "v": random.randrange(-v_bounds, v_bounds),
                }
            )
    ...
    def test_KF_descending(self):
        ...
        config = {"dt": 0.01, "save_freq": 0.01, "events": "zero"}

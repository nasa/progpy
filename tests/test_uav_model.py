...
from progpy.utils.traj_gen import Trajectory
from progpy.models.aircraft_model import SmallRotorcraft
from progpy.loading.controllers import LQR_I, LQR
...
class TestUAVGen(unittest.TestCase):
    ...
    def test_reference_trajectory_generation(self):
        ...
        # Define waypoints
        waypoints_dict = {}
        waypoints_dict["lat_deg"] = np.array(
            [
                37.09776,
                37.09776,
                37.09776,
        ...
        waypoints_dict["lon_deg"] = np.array(
            [
                -76.38631,
                -76.38629,
                -76.38629,
        ...
        waypoints_dict["alt_ft"] = np.array(
            [
                -1.9682394,
                164.01995,
                164.01995,
        ...
        etas_wrong = [
            dt.datetime.fromtimestamp(waypoints_dict["time_unix"][ii])
            for ii in range(len(waypoints_dict["time_unix"]))
        ]
        ...
        with self.assertRaises(ValueError):
            ref_traj = Trajectory(
                lat=waypoints_dict["lat_deg"],
                lon=waypoints_dict["lon_deg"][2:4],
                alt=alt_in,
                takeoff_time=takeoff_time,
                etas=etas_in,
            )
        ...
        FEET2MET = 0.3048
        coord = geom.Coord(
            lat0=waypoints_dict["lat_deg"][0],
            lon0=waypoints_dict["lon_deg"][0],
            alt0=waypoints_dict["alt_ft"][0] * FEET2MET,
        )
        x_ref, y_ref, z_ref = coord.geodetic2enu(
            waypoints_dict["lat_deg"] * np.pi / 180.0,
            waypoints_dict["lon_deg"] * np.pi / 180.0,
            waypoints_dict["alt_ft"] * FEET2MET,
        )
        time_ref = [
            waypoints_dict["time_unix"][iter] - waypoints_dict["time_unix"][0]
            for iter in range(len(waypoints_dict["time_unix"]))
        ]
    ...
    def test_controllers_and_vehicle(self):
        ...
        vehicle_djis = SmallRotorcraft(
            **{"dt": 0.1, "vehicle_model": "djis1000", "process_noise": 0}
        )
        ...
        with self.assertRaises(TypeError):
            # Incorrect type for values in reference trajectory dict
            ctrl = LQR(
                x_ref={"x": np.array([1, 2, 3]), "y": [1, 2, 3]}, vehicle=vehicle
            )
        with self.assertRaises(TypeError):
        ...
        sim = vehicle.simulate_to(
            ref_traj["t"][-1],
            ctrl,
            **{"dt": vehicle.parameters["dt"], "save_freq": vehicle.parameters["dt"]},
        )
        ...
        # Generate reference trajectory
        ref_traj_speeds_temp = Trajectory(
            lat=lat_in, lon=lon_in, alt=alt_in, **ref_params
        )
        ref_traj_speeds = ref_traj_speeds_temp.generate(
            dt=vehicle_djis.parameters["dt"]
        )
        ...
        # Simulate and compare:
        sim_speeds = vehicle_djis.simulate_to(
            ref_traj_speeds["t"][-1],
            ctrl_speeds,
            **{"save_freq": vehicle_djis.parameters["dt"]},
        )
        x_speeds_temp = [
            sim_speeds.outputs[iter]["x"] for iter in range(len(sim_speeds.times))
        ]
        y_speeds_temp = [
            sim_speeds.outputs[iter]["y"] for iter in range(len(sim_speeds.times))
        ]
        z_speeds_temp = [
            sim_speeds.outputs[iter]["z"] for iter in range(len(sim_speeds.times))
        ]
        x_speeds_sim_interp = interp1d(sim_speeds.times, x_speeds_temp)(
            ref_traj_speeds["t"]
        )
        y_speeds_sim_interp = interp1d(sim_speeds.times, y_speeds_temp)(
            ref_traj_speeds["t"]
        )
        z_speeds_sim_interp = interp1d(sim_speeds.times, z_speeds_temp)(
            ref_traj_speeds["t"]
        )
        for iter in range(len(ref_traj_speeds["t"])):
            self.assertAlmostEqual(
                ref_traj_speeds["x"][iter], x_speeds_sim_interp[iter], delta=8
            )
            self.assertAlmostEqual(
                ref_traj_speeds["y"][iter], y_speeds_sim_interp[iter], delta=8
            )
            self.assertAlmostEqual(
                ref_traj_speeds["z"][iter], z_speeds_sim_interp[iter], delta=8
            )
